...
14. **PACKAGE-DAC-001-IMPLEMENT-001** (was `PACKAGE-DAC-001` before
    ...
    - Status: **LANDED 2026-05-23 as PR #573 (package layer)** — the
      package YAML reconciliation is complete. Per
      `PRODUCT-DAC-001-READINESS-REFRESH` (this PR), the next PR is
      **`FW-COMPILE-DAC-001`** (compile-only validation of the
      `gp8403:` `voltage:` binding — the package feeds the neutral
      `0-10V` value, not ESPHome's bare `10V` / `5V` enum, and no
      FanDAC compile-only target exists) **before** `PRODUCT-DAC-001`.
      Product / WebFlash / release readiness remains blocked. The
...
15. **PRODUCT-DAC-001** (preceded by **PRODUCT-DAC-001-READINESS-REFRESH** /
    this PR / 2026-05-23, docs-only; recommends **`FW-COMPILE-DAC-001`**
    compile-only validation before the product YAML)
    - Status: Blocked on `FW-COMPILE-DAC-001` (compile-only validation
      of the `gp8403:` `voltage:` binding). `PACKAGE-DAC-001` landed at
      the package layer as **PR #573**, but `PRODUCT-DAC-001-READINESS-REFRESH`
      (this PR) found the package feeds `voltage:` the neutral `0-10V`
      value rather than ESPHome's bare `10V` / `5V` enum, and with no
      FanDAC compile-only target the compile is **unvalidated**. The
      recommended next PR is `FW-COMPILE-DAC-001` (compile-only
      validation; confirm `0-10V` validates or record the `0-10V` →
      `10V` fix). `PRODUCT-DAC-001` may alternatively carry the
      `0-10V` → `10V` mapping itself once the compile concern is
      resolved.
