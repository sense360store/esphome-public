...
8. **WEBFLASH-RELAY-001** (preceded by **WEBFLASH-RELAY-001-READINESS-REFRESH**)
    - Status: Blocked on production-wide hardware characterisation +
      installation / safety / competent-person sign-off +
      WebFlash-side manual-warning UX parity. `PRODUCT-RELAY-001`
      has now landed (this PR) but WebFlash exposure remains a
      separate gate per the readiness refresh recommended posture
      (`advanced/manual-warning-only`, not the default
      `preview-candidate`). The recommended next Relay chain PR is
      `WEBFLASH-RELAY-001-READINESS-REFRESH` (docs-only readiness
      re-evaluation that classifies WebFlash exposure as still
      `not-webflash-ready` even with the product YAML in place),
      not immediate `WEBFLASH-RELAY-001` implementation work.
    ...
    - Notes: Pairs with WebFlash-side `WF-IMPORT-RELAY-001`. The
      ...
      **resolved**; the `PACKAGE-RELAY-001` evidence layer is
      ...
      product YAML has now landed (this PR) under
      [`products/sense360-ceiling-poe-ventiq-fanrelay-roomiq.yaml`](products/sense360-ceiling-poe-ventiq-fanrelay-roomiq.yaml).
      Remaining gates are production-wide / multi-unit hardware
      characterisation; installation / safety / competent-person
      sign-off; WebFlash-side manual-warning UX parity; the
      WEBFLASH-RELAY-001 wrapper + catalog + build-matrix row
      itself. The `PACKAGE-RELAY-001-READINESS-REFRESH` PR
      (PR #559, docs-only) consolidated the readiness table at
      ...
      posture; `PRODUCT-RELAY-001` (this PR) landed the product
