...
- **CI-REFACTOR-VERIFY-001** delivers, via **this PR** on 2026-05-30, the
  ...
  **verification + docs only, with NO workflow edit**. Ran the actual CI sweep
  commands against the post-refactor layout and showed each passes as-is:
  `yamllint -c .yamllint products/ packages/` (exit 0, clean — recurses into
  `packages/boards/` + `products/bundles/`); the `find products/ … ! -path
  "products/webflash/*"` discovery (43 files — 7 bundles + 28 top-level
  shims/legacy + 8 compile-only; bundles auto-discovered, webflash excluded,
  every `!include` chain resolves); the `sed` targets intact
  (`packages/base/external_components.yaml` keeps `ref: main`; `packages/features/`
  keeps the 30 `@main` YAMLs — base/feature tiers untouched);
  `validate_compile_targets.py --metadata-only` (14 targets resolve through the
  shims/bundles); `validate_configs.py` (223 files / 0 failed);
  `test_release_one_entity_names.py`; `test_product_substitutions.py` (28
  products); and `python3 -m unittest discover -s tests -p "test_*.py"` (1154
  tests, 3 skipped). **Gate identity confirmed:** `validate.yml`
  (`name: Quick Validation`) and `firmware-build-release.yml`
  (`name: Build & Release Firmware`) `name:` fields unchanged — the whole
  `.github/workflows/` tree is byte-identical to `main` — so no
  branch-protection required-status-check name moved. No glob fix was needed
  (bundles live under `products/`, per §5; the §5.5 test repoints already
  travelled with `PACKAGE-RENAME-001..005`). Updated
  [`docs/ci-pipeline.md`](docs/ci-pipeline.md) (boards/bundles/aliases/shims
  layout + per-workflow handling + gate-identity table) and
  [`docs/arch-board-bundle-plan.md`](docs/arch-board-bundle-plan.md) §7 item 5.
  **Next: `DOCS-ARCH-REFRESH-001`** (the remaining §6.1 doc set), then
...
- **PACKAGE-RENAME-005** delivers, via **this PR** on 2026-05-30, the **FINAL
  ...
  **byte-identical**: the board-package functional body is
  ...
  stay OUT of the board layer by design. **CI-REFACTOR-VERIFY-001 (the
  gate-parity proof) is now done** (see the lead bullet above); the next slice
  is `DOCS-ARCH-REFRESH-001`. (Alias removal / consumer rebind onto the board
  packages stays a separate, later slice gated on each legacy binder count
  reaching zero via `PRODUCT-DEP-CORE-001`.)
- **PACKAGE-RENAME-004** delivers, via **this PR** on 2026-05-30, the **fourth
  ...
  **S360-200 RoomIQ family** (`presence_*` / `comfort_*`, the highest binder
  count, last sensor family per the §7 ordering). The SKU-aligned board package
  [`packages/boards/s360-200-roomiq.yaml`](packages/boards/s360-200-roomiq.yaml)
  (+ the wall board [`s360-200-roomiq-wall.yaml`](packages/boards/s360-200-roomiq-wall.yaml))
  is now the **authoritative** production-facing RoomIQ board. **Unlike the 1:1
  LED / AirIQ / VentIQ families, the RoomIQ board merges TWO drivers that
  products bind INDEPENDENTLY** (the legacy `sense360-core-ceiling*` / `-wall*`
  family `!include`s `comfort_*.yaml` and `presence_*.yaml` under separate
  package keys), so a single merged board file cannot be the alias target of
  both legacy paths without duplicate-id breakage. The RoomIQ board layer is
  therefore **authoritative PER DRIVER**: the authoritative, self-contained
  content lives in four SKU-aligned board halves —
  [`s360-200-roomiq-climate.yaml`](packages/boards/s360-200-roomiq-climate.yaml) /
  [`-radar.yaml`](packages/boards/s360-200-roomiq-radar.yaml) (ceiling) and
  [`-climate-wall.yaml`](packages/boards/s360-200-roomiq-climate-wall.yaml) /
  [`-radar-wall.yaml`](packages/boards/s360-200-roomiq-radar-wall.yaml) (wall) —
  and the two production-facing boards COMPOSE their two halves (bundle include
  path unchanged). The four content-holding legacy paths
  `packages/expansions/{comfort_ceiling,presence_ceiling,comfort_wall,presence_wall}.yaml`
  are reduced to thin `!include` **aliases** of the matching half — **paths
  preserved** per §3.3 (the legacy core ceiling/wall products, the RoomIQ
  bundles / compile-only targets, and the Phase-2 naming aliases still bind
  them; no alias dropped while a live binder exists). The shared radar
  **primitives** `presence_ld2450.yaml` / `presence_ld2412.yaml` are
  cross-referenced base drivers (documented in `packages/SENSE360_MODULES.md`,
  no board package holds their content) and stay **authoritative, NOT aliased**,
  mirroring how `airiq.yaml` (RENAME-002) and `ceiling_halo_leds.yaml`
  (RENAME-001) stayed authoritative — noted in the board header. The Phase-2
  naming aliases [`roomiq.yaml`](packages/expansions/roomiq.yaml) /
  [`roomiq_radar.yaml`](packages/expansions/roomiq_radar.yaml) keep their pinned
  bare-basename `!include` of `comfort_ceiling.yaml` / `presence_ceiling.yaml`
  (now themselves board aliases) so `tests/test_roomiq_alias_packages.py` stays
  green and the chains resolve identically. The three zero-binder S3/module
  legacy files (`presence_ceiling_s3.yaml`, `comfort_ceiling_s3.yaml`,
  ...
  lossy and **not** byte-identical (recorded, not silently resolved). The
  content-asserting check that travels with the moved RoomIQ content per §5.5 is
  **repointed** in the same slice — `test_core_abstract_bus.py`'s
  `COMFORT_CEILING_PACKAGE` constant and the comfort ceiling/wall entries of
  `SHARED_I2C_CONSUMER_DEFAULTS` now read the board halves — assertions intact,
  only the path read follows the content. Resolution proven **byte-identical**:
  each board-half body is character-for-character equal to the pre-flip legacy
  file body (sha256-matched), and the full include-graph payload is identical
  HEAD vs working tree for the production product (`Ceiling-POE-VentIQ-RoomIQ`),
  the legacy core ceiling/wall products, the RoomIQ bundles, and a RoomIQ
  compile-only target. **No** config-string / artifact / WebFlash
  (`config/webflash-builds.json`, `manifest.json`, `firmware/sources.json`) /
  readiness / lifecycle / `schematic_status` change;
  ...
  **pending-ci** (ESPHome unavailable; no compile result claimed). Full suite
  green (1154 tests, 3 skipped). **The sensor-family renames are now complete**
  (LED, AirIQ, VentIQ, RoomIQ); remaining in the epic are the **PSU**
  (`power_poe` → `S360-410`) and the **deferred mains driver** boards
  (`S360-310` / `320` / `400`, compliance-gated), then **CI-REFACTOR-VERIFY-001**.
- **PACKAGE-RENAME-003** delivers, via **this PR** on 2026-05-30, the **third
  source-of-truth flip** of the §7-item-4 rename epic: the **S360-211 VentIQ
  family** (`docs/arch-board-bundle-plan.md` §7 item 4 ordering — VentIQ after
  AirIQ; note the `airiq_bathroom_*` pair is VentIQ despite the prefix). The
  SKU-aligned board package
  [`packages/boards/s360-211-ventiq.yaml`](packages/boards/s360-211-ventiq.yaml)
  plus its `-pro` hardware-variant overlay
  [`s360-211-ventiq-pro.yaml`](packages/boards/s360-211-ventiq-pro.yaml) now hold
  the **authoritative, self-contained** VentIQ definition (consolidating the
  content that previously lived in the legacy
  `packages/expansions/airiq_bathroom_base.yaml` / `airiq_bathroom_pro.yaml`
  drivers), and those two content-holding legacy paths are reduced to thin
  `!include` **aliases** of the board packages — **paths preserved** per §3.3
  (`products/sense360-core-ceiling-bathroom.yaml`, the FanRelay/FanTRIAC bundles,
  the VentIQ compile-only target, and the Phase-2 naming aliases still bind
  `airiq_bathroom_base.yaml`; no alias is dropped while a live binder exists,
  alias removal is a later slice gated on binder count = 0). **Unlike AirIQ,
  the entire VentIQ driver content is folded** — there is **no** un-folded
  generic base driver (no VentIQ `airiq.yaml`/`ceiling_halo_leds.yaml`
  equivalent) to keep authoritative; the whole driver lived in the
  `airiq_bathroom_*` pair, both folded. The compat wrapper
  [`packages/expansions/bathroom.yaml`](packages/expansions/bathroom.yaml) is
  **repointed** to bind the board package directly (board hardware +
  `bathroom_profile` feature, mirroring the production bundle), while the
  Phase-2 naming aliases
  [`ventiq.yaml`](packages/expansions/ventiq.yaml) /
  [`ventiq_extended.yaml`](packages/expansions/ventiq_extended.yaml) keep their
  pinned bare-basename `!include` of `airiq_bathroom_base.yaml` /
  `airiq_bathroom_pro.yaml` (now themselves board aliases) so
  `tests/test_ventiq_alias_packages.py` stays green and the chains resolve
  identically. The content-asserting check that travels with the moved VentIQ
  content per §5.5 is **repointed** in the same slice — the two `bathroom_i2c_id`
  entries of `test_core_abstract_bus.py`'s `SHARED_I2C_CONSUMER_DEFAULTS` now
  read the board packages — assertion intact, only the path read follows the
  content. Resolution proven **byte-identical**: each board-package body is
  character-for-character equal to the pre-flip legacy file body (sha256-matched),
  and the full production (`Ceiling-POE-VentIQ-RoomIQ`) include-graph content
  sequence is identical HEAD vs working tree (also verified for the other live
  binders). **No** config-string / artifact / WebFlash
  (`config/webflash-builds.json`, `manifest.json`, `firmware/sources.json`) /
  readiness / lifecycle / `schematic_status` change;
  `test_release_one_entity_names` and `test_product_substitutions` untouched; no
  workflow edit; WebFlash repo untouched; full `esphome compile` is
  **pending-ci** (ESPHome unavailable; no compile result claimed). Full suite
  green (1154 tests, 3 skipped). **Next slice: PACKAGE-RENAME-004 (RoomIQ —
...
- **BUNDLE-LAYER-002** delivers, via **this PR** on 2026-05-30, the **rest of
  ...
  `ceiling-poe-fandac`, `ceiling-poe-ventiq-fanrelay-roomiq`,
  `ceiling-poe-ventiq-fantriac-roomiq`), each carrying the moved product
  composition verbatim (same substitutions, entity names, config string,
  artifact-name identity, and header wording; include depth `../packages` →
  `../../packages`), with the five `products/sense360-*.yaml` config-string
  products converted to thin **compat shims** (§3.2), paths preserved. The
  structural / fan-readiness tests that read the now-moved product-internal
  layout (`test_{pwm,dac,relay}_product_readiness`, `test_native_fan_gpio_map`,
  `test_fan_pwm_package`, `test_fan_relay_package`, `test_fandac_package`,
  `test_compile_targets`) are **repointed** onto the bundle files — assertions
  preserved, only the path read follows the moved content; catalog /
  compile-only-target path-identity references stay on the product shim. The two
  shipping-identity tests (`test_release_one_entity_names`,
  `test_product_substitutions`) and the two LAYER-001 shipping bundles + shims
  are **untouched**. Resolution byte-identical; the WebFlash contract (config
  strings, artifact names, `config/webflash-builds.json`, `manifest.json`,
  `firmware/sources.json`) unchanged; no lifecycle / `schematic_status` change,
  LED not marked stable, `S360-410` not verified, no fan blocker closed,
  FanTRIAC stays `blocked` / `HW-005`, WebFlash repo untouched; full `esphome
  compile` is **pending-ci** (ESPHome unavailable; no compile result claimed).
  Full suite green (1154 tests, 3 skipped). **The bundle layer is now
  complete** (all seven config strings bundled + shimmed). Next phase:
  **PACKAGE-RENAME-00x**, then **CI-REFACTOR-VERIFY-001**.
- **BUNDLE-LAYER-001** delivers, via **this PR** on 2026-05-30, the **bundle
  layer for the two WebFlash-shipping config strings**: a new
  `products/bundles/` directory holding the config-string-named bundles
  [`ceiling-poe-ventiq-roomiq.yaml`](products/bundles/ceiling-poe-ventiq-roomiq.yaml)
  (production / stable) and
  [`ceiling-poe-ventiq-roomiq-led.yaml`](products/bundles/ceiling-poe-ventiq-roomiq-led.yaml)
  (preview), each assembling the §2.2 board stack from `packages/boards/` plus
  the base tier and feature profiles, with the two shipping products converted
  to thin **compat shims** (§3.2) that do nothing but `!include` their bundle.
  Resolution is proven identical (leaf functional packages, merged
  substitutions, `text_sensor` identity all byte-equal pre/post); the WebFlash
  contract (config strings, artifact names, `config/webflash-builds.json`,
  `manifest.json`, `firmware/sources.json`) is **byte-identical** and the
  `products/webflash/* → products/sense360-<stem>.yaml → bundle` chain still
  resolves to real files. **Scoped to the shipping pair:** the five
  **non-shipping** config strings (FanRelay / FanTRIAC / FanDAC / FanPWM /
  RoomIQ-only) are **deferred to BUNDLE-LAYER-002** because their package-/
  product-layer guardrail tests (`test_fandac_package`, `test_fan_relay_package`,
  `test_{relay,pwm,dac}_product_readiness`) assert on the product-internal
  content the bundle move would relocate, and §5.5 requires those test repoints
  to travel in the same slice as the move. **No** workflow / `config/*.json` /
  test edit, **no** lifecycle / `schematic_status` change, LED not marked
  stable, `S360-410` not verified, no fan blocker closed, WebFlash repo
  untouched; full `esphome compile` is **pending-ci** (ESPHome unavailable; no
  compile result claimed). Full suite green (1154 tests, 3 skipped). Next phase:
  **BUNDLE-LAYER-002** (remaining five bundles + shims + test repoints), then
  **PACKAGE-RENAME-00x** and **CI-REFACTOR-VERIFY-001**.
...
- **S360-100-CONNECTOR-PINMAP-001** delivers, via **PR #638** on
  ...
  module SKU + friendly name, connector type, pin count, pin-1
  orientation, intended function, and per-pin tables with the
  columns Pin number / Core net / ESP32 GPIO / Module-side
  signal / Signal type / Voltage / Status. The canonical R4
  schematic ([`docs/hardware/schematics/S360-100-R4.pdf`](docs/hardware/schematics/S360-100-R4.pdf),
  SHA256 `4c9e8b06d129fbb55f61e143b648e03762d06cb4dc67fe3120c268cd3a4bdf16`,
  837,443 bytes) is the per-net / per-GPIO source of truth; per-
  module audit docs are the per-board source of truth; this
  document sits one level up and consolidates them.
  **Change:** a new
  [`docs/hardware/s360-100-core-connector-pin-map.md`](docs/hardware/s360-100-core-connector-pin-map.md)
  records (a) the canonical connector matrix listing every Core
  connector (`J1` / `J2` / `J3` / `J4` / `J6` / `J7` / `J9` /
  `J10` / `J13` / `J15`) against its attached Sense360 module SKU
  (`S360-200` / `S360-210` / `S360-211` / `S360-300` / `S360-310` /
  `S360-311` / `S360-312` / `S360-320` / `S360-410`); (b) per-
  connector pin tables with the seven-column schema above; (c) the
  status-language rules (`verified` / `schematic-backed` / `TBD` /
  `needs silkscreen confirmation`) — every row carries one of the
  four values, with `schematic-backed` dominating where the
  canonical R4 PDF prints the net but no silkscreen / bench
  evidence yet, and `TBD` reserved for items the schematic does
  not unambiguously print; (d) the native-GPIO architectural rule
  restated for tach / pulse counter (`TachIO`→`IO16`,
  `Pul_Cou1..4`→`IO17`/`IO18`/`IO46`/`IO9`, no row maps a tach
  signal through an expander); (e) the historical SX1509 fan-path
  classification (legacy / superseded) preserved with link-outs to
  S360-100-NATIVE-FAN-GPIO-MAP-001 and
  S360-100-NATIVE-TACH-PULSE-001; (f) an Open Questions list for
  J1-vs-J9 (VentIQ), J6 pins 11/12 (UART or unconnected on Core
  side), J13 pin count (2-pin vs 3-pin cross-doc tension),
  `TRI_GPIO1` / `TRI_GPIO2` ESP32-side source pins (FanTRIAC
  HW-005), and the silkscreen / connector-part-number opens; and
  (g) the do-not-change guardrails. Cross-links added in
  [`docs/hardware/s360-100-r4-core.md`](docs/hardware/s360-100-r4-core.md)
  (See also row),
  [`docs/hardware/s360-311-r4-pwm.md`](docs/hardware/s360-311-r4-pwm.md)
  (See also row),
  [`docs/sense360-room-bundles.md`](docs/sense360-room-bundles.md)
  (Core-as-central-hub framing), and
  [`docs/blocker-burndown.md` §2A `PWM-12`](docs/blocker-burndown.md)
  (canonical Core-to-module pin map cross-reference appended).
  **Tests:** a new
  [`tests/test_s360_100_core_connector_pin_map.py`](tests/test_s360_100_core_connector_pin_map.py)
  pins (a) the canonical pin-map doc exists and self-identifies
  as S360-100-CONNECTOR-PINMAP-001; (b) the connector matrix
  lists every expected module SKU with its friendly name and
  every expected connector ref (`J1` / `J2` / `J3` / `J4` / `J6` /
  `J7` / `J9` / `J10` / `J13` / `J15`); (c) connector refs are
  unique in the matrix; (d) the per-connector pin tables include
  the seven required columns and every pin-table row ends with one
  of the four allowed Status values; (e) every tach /
  pulse-counter / PWM-drive net carries its canonical native
  ESP32-S3 GPIO termination (`TachIO`→`IO16`, etc.) and no row
  maps a tach signal through the SX1509 expander; (f) the
  do-not-change guardrails (`S360-310` / `S360-311` / `S360-312` /
  `S360-320` / `S360-410` stay `cataloged_unverified`; FanPWM
  stays out of `config/webflash-builds.json`; FanPWM products
  keep `rpm_supported: false` and `webflash_build_matrix: false`;
  the doc contains no release-readiness phrasing and does not
  claim FanTRIAC HW-005 or PACKAGE-POE-410-001 resolved); and
  (g) cross-doc linking from the Core reference doc, the PWM
  module audit, the room-bundles doc, and `UPCOMING_PR.md`. The
  pre-existing
  [`tests/test_s360_100_core_architecture.py`](tests/test_s360_100_core_architecture.py)
  and
  [`tests/test_native_fan_gpio_map.py`](tests/test_native_fan_gpio_map.py)
  remain the authoritative guards for the Core architecture index
  and the FanPWM native GPIO map respectively.
  **No** firmware publish, **no** release artifact, **no**
  `firmware/sources.json` change, **no** `manifest.json` change,
  **no** release target promoted, **no** LED promoted from
  preview to stable, **no** FanRelay / FanPWM / FanDAC release
  promotion, **no** FanTRIAC `HW-005` resolution claim, **no**
  S360-410 PoE PSU blocker resolution claim, **no** measured
  RPM / tach / PWM / current / thermal claim, **no** WebFlash
  build / `artifact_name` flip, **no** invented connector type /
  pin order / signal assignment / GPIO allocation (values that
  are not proven by the canonical R4 schematic carry `TBD` and
  `needs silkscreen confirmation`), **no** firmware YAML edit,
  **no** deletion or weakening of the historical SX1509 +
  `pulse_counter` compile/config-proof fixture / test.

- **S360-100-NATIVE-FAN-GPIO-MAP-001** delivers, via **PR #637** on
...
- **STABLE-RELEASE-MATRIX-ALL-YAML-001** delivers, via **this PR** on
  ...
  product YAMLs, [`products/webflash/`](products/webflash/) wrappers,
  and [`products/compile-only/`](products/compile-only/) skeletons),
  filling the gap that `RELEASE-PIPELINE-ROOM-MATRIX-001` / PR #621 and
  `RELEASE-NOTES-PIPELINE-001` / PR #623 left because both were scoped
  to the RoomIQ / LED room-firmware family only. **Change:** a new
  [`docs/all-yaml-release-matrix.md`](docs/all-yaml-release-matrix.md)
  enumerates every YAML in the repo and classifies it into exactly one
  of six release classes — `stable-release`, `preview-release`,
  `manual-candidate-only`, `compile-only`, `blocked`, or
  `not-a-product-entrypoint` — with the hardware boards required, the
  catalog status, the compile / WebFlash-wrapper / `artifact_name` /
  release-channel / release-selectability state, the blockers, and
  whether the YAML should appear in release-note / product-selection
  workflows. The doc also defines the explicit **stable-promotion
  criteria** (product YAML exists, catalog row exists, top-level full
  compile validated, WebFlash wrapper exists if WebFlash release is
  required, `artifact_name` exists, release notes can be generated,
  no blocking hardware / compliance caveat, not currently
  manual-candidate-only, not currently preview-only) and the
  **candidate stable additions** audit (none is approved by this PR;
  each row records exactly which evidence is missing). A new
  read-only classifier
  [`scripts/classify_all_yaml_release_matrix.py`](scripts/classify_all_yaml_release_matrix.py)
  derives the table from the source-of-truth files
  ([`config/webflash-builds.json`](config/webflash-builds.json),
  [`config/product-catalog.json`](config/product-catalog.json),
  [`config/manual-firmware-artifacts.json`](config/manual-firmware-artifacts.json),
  [`config/compile-only-targets.json`](config/compile-only-targets.json))
  with the same FanRelay / FanPWM / FanDAC token guardrail that
  ...
  (28 tests) locks in: every YAML in `products/` is classified
  exactly once into one of the six known classes; the release-selectable
  set equals `config/webflash-builds.json` (release selection derived
  from the source of truth, not hardcoded to one config); exactly one
  stable target today (`Ceiling-POE-VentIQ-RoomIQ`); exactly one
  preview target today (`Ceiling-POE-VentIQ-RoomIQ-LED`); LED is
  preview-only (artifact carries `-preview`, not `-stable`; not in
  the stable-targets list); FanRelay / FanPWM / FanDAC are
  `manual-candidate-only`, never release-selectable, never carry an
  `artifact_name`, and no fan family token may leak into the release
  matrix; FanTRIAC is `blocked`; every `products/compile-only/*.yaml`
  is `compile-only` and not release-selectable; WebFlash wrappers and
  `products/secrets.yaml` are `not-a-product-entrypoint`; the
  generator does not hardcode "Release-One firmware" wording for the
  LED preview target. **Confirmed:** 48 YAMLs classified (1
  `stable-release`, 1 `preview-release`, 3 `manual-candidate-only`, 7
  `compile-only`, 1 `blocked`, 35 `not-a-product-entrypoint`); the
  release-selectable set is exactly `{Ceiling-POE-VentIQ-RoomIQ,
  Ceiling-POE-VentIQ-RoomIQ-LED}`, mirroring
  [`config/webflash-builds.json`](config/webflash-builds.json); no
  other room combo is approved for promotion in this PR (each
  candidate's exact blockers are listed in the doc). **Validation
  (all passed):** `python3 scripts/classify_all_yaml_release_matrix.py`,
  `python3 scripts/plan_room_release_notes.py`,
  `python3 tests/test_all_yaml_release_matrix.py` (28 tests),
  ...
  `python3 scripts/validate_compile_targets.py --metadata-only` (12
  targets), `python3 tests/validate_webflash_builds.py` (2 builds),
  `python3 tests/test_product_catalog.py` (41 tests),
  `python3 tests/test_workflow_permissions.py` (7 tests), and
  `python3 -m unittest discover -s tests -p "test_*.py"` (954 tests,
  3 skipped — the prior baseline was 926 tests, +28 from the new
  contract tests). **Scope:** the new doc, the new classifier script,
  the new contract-test file, and this `UPCOMING_PR.md` update.
  **Does not:** publish a GitHub Release; commit any `.bin` /
  checksum / build-info file; edit any YAML under `products/**` or
  `products/webflash/**`; promote LED from `preview` to `stable`;
  promote FanRelay / FanPWM / FanDAC out of manual-candidate-only;
  add a fan `artifact_name`; flip any `webflash_build_matrix` value;
  add or remove any entry in `config/webflash-builds.json` /
  `config/product-catalog.json` /
  `config/compile-only-targets.json` /
  `config/manual-firmware-artifacts.json`; write
  `firmware/sources.json` or `manifest.json`; invent unsupported
  YAML / product combos; treat compile-only skeletons as release
  products; or claim WebFlash / import / release / hardware-stable /
  compliance readiness for any fan candidate. **No** fabricated
  evidence.
...
- **RELEASE-WORKFLOW-DRYRUN-GATE-FIX-001** delivers, via **this PR** on
  ...
  the dry-run mode (workflow run `26558131655`, commit
  `f6fe43366fbf3e70013e8189fbe8f49848fc7a82`) failed at `Generate Build
  Matrix` / `Generate product build matrix` because that job had no `if:`
  gate and ran on every `workflow_dispatch` against the default
  `version=0.0.0-dev` / `channel=preview` (no matching
  [`config/webflash-builds.json`](config/webflash-builds.json) entry), and at
  `Release Dry-Run` / `Verify dry-run guardrails (planner contract tests)`
  because the read-only dry-run job did not install PyYAML before
  [`tests/test_release_dry_run_mode.py`](tests/test_release_dry_run_mode.py)
  (which parses the workflow YAML) ran. **Change:**
  [`.github/workflows/firmware-build-release.yml`](.github/workflows/firmware-build-release.yml)
  now gates `generate-matrix`, `build`, and `summary` to
  `github.event_name == 'release' || (github.event_name == 'workflow_dispatch'
  && !inputs.dry_run)` so a `workflow_dispatch` with `dry_run=true` only runs
  the `release-dry-run` job, and the `release-dry-run` job adds
  `pip install pyyaml` before the planner contract tests. **Test:**
  [`tests/test_release_dry_run_mode.py`](tests/test_release_dry_run_mode.py)
  gains `DryRunGatingTests` locking in: `generate-matrix` / `build` /
  `summary` all require `!inputs.dry_run` on a `workflow_dispatch` and still
  allow `github.event_name == 'release'`; the `release` (publish) job gate
  stays `github.event_name == 'release'` with no `workflow_dispatch` /
  `dry_run` reference; and the dry-run job installs PyYAML. **Publishing
  stays gated to a real `release` event** — no new `softprops/action-gh-release`
  step, no new `contents: write` grant, no `firmware/sources.json` /
  `manifest.json` write, no `.bin` / checksum commit, and no `products/**` /
  `products/webflash/**` edit. **Confirmed:** dry-run still plans
  `Ceiling-POE-VentIQ-RoomIQ` (`stable`) + `Ceiling-POE-VentIQ-RoomIQ-LED`
  (`preview`) only; FanRelay / FanPWM / FanDAC excluded; FanTRIAC blocked
  (HW-005). **Validation (all passed):**
  `python3 scripts/plan_room_release_notes.py`,
  ...
  `python3 tests/test_workflow_permissions.py`, and
  `python3 -m unittest discover -s tests -p "test_*.py"` (888 tests, 3
  skipped). **Scope:** the workflow gate change + the new test class + doc
  updates to [`docs/room-firmware-release-notes.md`](docs/room-firmware-release-notes.md),
  [`docs/release-artifact-readiness-matrix.md`](docs/release-artifact-readiness-matrix.md),
  and this file. **Does not** publish a GitHub Release; commit any `.bin` /
  checksum; edit `products/**` or `products/webflash/**`; add a fan
  `artifact_name`; flip any fan `webflash_build_matrix`; write
  `firmware/sources.json` or `manifest.json`; include FanRelay / FanPWM /
  FanDAC in release notes; or claim fan release / WebFlash / hardware-stable
  / compliance readiness. **No** fabricated evidence.

- **RELEASE-WORKFLOW-DRYRUN-MODE-001** delivers, via **this PR** on 2026-05-27,
  an explicit, **safe-by-default dry-run mode** for the room firmware release
  workflow — the "next input" that `RELEASE-CI-DRYRUN-001` / PR #624 defined but
  did not implement. **Change:**
  [`.github/workflows/firmware-build-release.yml`](.github/workflows/firmware-build-release.yml)
  gains a `workflow_dispatch` boolean input `dry_run` (**default `true`**, safe /
  non-publishing) and a new **read-only** `release-dry-run` job
  (`permissions: contents: read`) gated
  `if: github.event_name == 'workflow_dispatch' && inputs.dry_run`. The job runs
  [`scripts/plan_room_release_notes.py`](scripts/plan_room_release_notes.py)
  (release-note planning + validation + fan-exclusion guardrail), then the
  planner contract tests
  ([`tests/test_plan_room_release_notes.py`](tests/test_plan_room_release_notes.py)
  and the new
  [`tests/test_release_dry_run_mode.py`](tests/test_release_dry_run_mode.py)),
  and asserts no release side-effect file was produced. **Confirmed:** dry-run
  plans stable `Ceiling-POE-VentIQ-RoomIQ` + preview `Ceiling-POE-VentIQ-RoomIQ-LED`
  only; FanRelay / FanPWM / FanDAC excluded; FanTRIAC blocked (HW-005).
  **Publishing stays gated to a real `release` event** — the `release` job's
  `if: github.event_name == 'release'` is unchanged and does not reference
  `workflow_dispatch` / `dry_run`; `softprops/action-gh-release` appears only in
  that job. **Validation (all passed):**
  `python3 scripts/plan_room_release_notes.py`,
  `python3 tests/test_plan_room_release_notes.py`,
  `python3 tests/test_release_dry_run_mode.py`,
  `python3 tests/validate_configs.py`,
  ...
  `python3 tests/validate_webflash_builds.py`,
  `python3 tests/test_workflow_permissions.py`, and
  `python3 -m unittest discover -s tests -p "test_*.py"` (883 tests, 3 skipped).
  **Scope:** the workflow input + new dry-run job, the new test, and doc updates
  to [`docs/room-firmware-release-notes.md`](docs/room-firmware-release-notes.md),
  [`docs/room-firmware-release-matrix.md`](docs/room-firmware-release-matrix.md),
  [`docs/release-artifact-readiness-matrix.md`](docs/release-artifact-readiness-matrix.md),
  and this file. **Does not** publish a GitHub Release; commit any `.bin` /
  checksum; edit `products/webflash/**`; add a fan `artifact_name`; flip any fan
  `webflash_build_matrix`; write `firmware/sources.json` or `manifest.json`;
  include FanRelay / FanPWM / FanDAC in release notes; or claim fan release /
  WebFlash / hardware-stable / compliance readiness. **No** fabricated evidence.
...
- **S360-311-CURRENT-THERMAL-EVIDENCE-REQUEST-001** delivers, via **this
  ...
  (`PWM-6` per-channel + aggregate current, `PWM-13` thermal) — which
  `PWM-BLOCKER-RECLASSIFY-001` (PR #602) scoped as gating **only** WebFlash
  exposure / release / hardware-stable / the production electrical-margin
  claim, **not** clean repo / YAML / firmware work — into one
  **operator-answerable fill-in checklist** (board rev; fan model + label
  current rating; supply voltage + **measured supply current**;
  **per-channel current at full speed**; **aggregate current with all four
  channels active**; **inrush / startup current** if measurable; **MT3608
  input / output current or measured ceiling**; **thermal method** — IR
  thermometer / thermal camera / thermocouple / qualitative only; **test
  duration**; **ambient temperature**; **hottest component / location**;
  **measured max temperature**; whether all four channels ran at high / full
  duty; whether any **voltage drop, instability, or reset** occurred;
  **`TachIO`/`GPIO16` observation only if tested — no RPM claim unless
  separately proven**; photos / videos / logs if available; operator / date /
  source / provenance) plus a **pass/fail evidence contract** (what closes
  per-channel current / aggregate current / MT3608 ceiling / electrical
  margin / measured thermal, and what stays out-of-scope: RPM, WebFlash
  exposure, release artifact, import readiness, hardware-stable promotion,
  compliance) in
  [`docs/hardware/s360-311-r4-pwm.md`](docs/hardware/s360-311-r4-pwm.md)
  §S360-311-CURRENT-THERMAL-EVIDENCE-REQUEST-001 (canonical),
  [`docs/blocker-burndown.md`](docs/blocker-burndown.md) §5D + §4 pointer,
  ...
  `inrush` / `MT3608` / `bench` / `fan` and spreadsheets / PDFs / photos and
  found **no measurement artifact** — only **design / CAD** material (the
  `12vFan_PWM_PulseCounter` set incl. `PWM.png`, the canonically-named
  `S360-311-R4` Drive folder owned by `kanyugistash@gmail.com` created
  2026-05-16 with KiCad sources / gerbers / CPL / STEP / BOM / schematic PDF /
  renders, and the unchanged `Sense360_R4_Tracker`), recorded for provenance
  only, committing no Drive file. **Recommended next PWM PR:**
  `S360-311-CURRENT-THERMAL-001` — **gated until the operator supplies the
  measured rows** (no measured-current / thermal row can close in a docs-only
  pass); WebFlash wrapper / build PRs (`WEBFLASH-PWM-001` / `RELEASE-PWM-001`
  / `WF-IMPORT-PWM-001`) stay blocked, and `WEBFLASH-PWM-LIVE-CHECK-001`
  stays queued behind WebFlash access. Edits are confined to `docs/**` and
  this file; **no** `packages/**`, `products/**`, `products/webflash/**`,
  `config/**`, `components/**`, `include/**`, `firmware/**`, `manifest.json`,
  `firmware/sources.json`, release artifact, checksum, `.github/workflows/**`,
  test, or `sense360store/WebFlash` edit; **no** product YAML, WebFlash
  wrapper, `webflash_build_matrix` flip, `artifact_name`, or release
  artifact; **no** measured-current / measured-thermal / RPM / WebFlash /
  import / release / compliance / hardware-stable claim; no `schematic_status`
...
- **WEBFLASH-PWM-001-READINESS** delivers, via **this PR** on 2026-05-26,
  ...
  product-validation and `WEBFLASH-DRIFT-001` / PR #595 drift-audit
  evidence — **without exposing FanPWM to WebFlash**. It adds a
  re-evaluated **PWM WebFlash readiness table** (gate / status / evidence /
  next action) to
  [`docs/webflash-exposure-readiness-matrix.md`](docs/webflash-exposure-readiness-matrix.md)
  §PWM / S360-311 WebFlash posture and audit-log / pointer notes to
  [`docs/release-artifact-readiness-matrix.md`](docs/release-artifact-readiness-matrix.md),
  [`docs/product-readiness-matrix.md`](docs/product-readiness-matrix.md), and
  [`docs/webflash-drift-audit.md`](docs/webflash-drift-audit.md) (§4.3
  follow-up resolution). Findings: the package / product / full-compile /
  `compile_validation_status` / `rpm_supported` / product-YAML-composition
  parity (FW-COMPILE-PWM-PRODUCT-001 / PR #594) / config-posture-hardware
  gates are `CLOSED` (product YAML
  [`products/sense360-ceiling-poe-fanpwm.yaml`](products/sense360-ceiling-poe-fanpwm.yaml),
  package [`packages/expansions/fan_pwm.yaml`](packages/expansions/fan_pwm.yaml)
  → `fan_pwm_sx1509.yaml`, full compile green in run `26414398902`,
  `validated-full-compile` + `rpm_supported: false`, prior-recorded — this
  is **stronger** than FanRelay, which has no compile-flag gap to resolve
  for PWM); WebFlash wrapper / `config/webflash-builds.json` row /
  `artifact_name` / release artifact / import-source / no-RPM UX are
  `BLOCKING`; live `sense360store/WebFlash` re-verification, the still-owed
  `S360-311` module-availability classification (`WEBFLASH-DRIFT-001`
  row #16), and the live product-YAML `esphome config` are `NEEDS-TOOLING`
  (read / ESPHome access unavailable this session); the **PWM polarity**,
  **per-fan / aggregate current + thermal envelope**, **product bench**,
  `TachIO` / `GPIO16` reserved/pending, and board-level thermal / EMI
  caveats remain `NEEDS-OPERATOR-INPUT` (the board is SELV — no mains — so
  the `COMPLIANCE-001` mains gate does not apply to FanPWM).
  **Recommended next PWM PR: `WEBFLASH-PWM-LIVE-CHECK-001`** (re-run the
  live WebFlash readiness / drift check once read access is restored;
  record the `S360-311` classification), with **`S360-311-BENCH-001`** (PWM
  polarity + per-fan / aggregate current + thermal envelope + product
  bench) as the substantive evidence gate — **not**
  `WEBFLASH-PWM-002-WRAPPER-PLAN` (non-WebFlash gates are not all clean)
  and **not** `WEBFLASH-PWM-001`. No `config/`, `packages/`, `products/`,
  `products/webflash/`, `config/webflash-builds.json`, test, workflow,
  firmware, `manifest.json`, `firmware/sources.json`, or
  `sense360store/WebFlash` edit; no `webflash_build_matrix` flip; no
  `artifact_name`; no RPM added or claimed; no `schematic_status` promotion
  (`S360-311` stays `cataloged_unverified`); Release-One
  (`Ceiling-POE-VentIQ-RoomIQ` / `v1.0.0` / `stable`), LED preview, and
  FanTRIAC (`blocked` / `HW-005`) unchanged. No WebFlash exposure / import /
  release / RPM-support / compliance / hardware-stable claim; no fabricated
  WebFlash evidence. See queue item 31.
- **WEBFLASH-DAC-001-READINESS** delivers, via **this PR** on 2026-05-26,
  a **docs-only** re-evaluation of FanDAC / S360-312 WebFlash-exposure
  ...
  FanDAC to WebFlash**. It adds a re-evaluated **DAC WebFlash readiness
  table** (gate / status / evidence / next action) to
  [`docs/webflash-exposure-readiness-matrix.md`](docs/webflash-exposure-readiness-matrix.md)
  §DAC / S360-312 WebFlash posture and audit-log / pointer notes to
  [`docs/release-artifact-readiness-matrix.md`](docs/release-artifact-readiness-matrix.md),
  [`docs/product-readiness-matrix.md`](docs/product-readiness-matrix.md), and
  [`docs/webflash-drift-audit.md`](docs/webflash-drift-audit.md) (§4.2
  follow-up resolution). Findings: the package / product / full-compile /
  `compile_validation_status` / `voltage`-enum / config-posture-hardware /
  `FanDAC ↔ AirIQ` mutex gates are `CLOSED` (product YAML
  [`products/sense360-ceiling-poe-fandac.yaml`](products/sense360-ceiling-poe-fandac.yaml),
  package [`packages/expansions/fan_dac.yaml`](packages/expansions/fan_dac.yaml)
  → `fan_gp8403.yaml`, full compile green in run `26364679370`,
  `validated-full-compile` + `voltage_enum_fixed: true`, prior-recorded —
  this is **stronger** than FanRelay, which has no compile-flag gap to
  resolve for DAC); WebFlash wrapper / `config/webflash-builds.json` row /
  `artifact_name` / release artifact / import-source / voltage-mode UX /
  compliance are `BLOCKING`; live `sense360store/WebFlash` re-verification
  and the still-owed `S360-312` module-availability classification
  (`WEBFLASH-DRIFT-001` row #17) are `NEEDS-TOOLING` (read access denied
  this session); the `J3` silkscreen transposition and Cloudlift S12
  harness / product-bench caveats remain `NEEDS-OPERATOR-INPUT`.
  **Recommended next DAC PR: `WEBFLASH-DAC-LIVE-CHECK-001`** (re-run the
  live WebFlash readiness / drift check once read access is restored;
  record the `S360-312` classification) — **not**
  `WEBFLASH-DAC-002-WRAPPER-PLAN` (non-WebFlash gates are not all clean)
  and **not** `WEBFLASH-DAC-001`. No `config/`, `packages/`, `products/`,
  `products/webflash/`, `config/webflash-builds.json`, test, workflow,
  firmware, `manifest.json`, `firmware/sources.json`, or
  `sense360store/WebFlash` edit; no `webflash_build_matrix` flip; no
  `artifact_name`; no `schematic_status` promotion (`S360-312` stays
  `cataloged_unverified`); no COMPLIANCE-001 movement; the
  `FanDAC ↔ AirIQ` mutex is not relaxed; Release-One
  (`Ceiling-POE-VentIQ-RoomIQ` / `v1.0.0` / `stable`), LED preview, and
  FanTRIAC (`blocked` / `HW-005`) unchanged. No WebFlash exposure / import /
  release / compliance / hardware-stable claim; no fabricated WebFlash
  evidence. See queue item 30.
...
| RELEASE-WORKFLOW-DRYRUN-MODE-001 | (this PR) | esphome-public | Merged — workflow dry-run mode (no publish) | Added an explicit, safe-by-default dry-run mode to [`.github/workflows/firmware-build-release.yml`](.github/workflows/firmware-build-release.yml): a `workflow_dispatch` boolean input `dry_run` (**default `true`**, non-publishing) and a new **read-only** `release-dry-run` job (`permissions: contents: read`, gated `github.event_name == 'workflow_dispatch' && inputs.dry_run`) that runs [`scripts/plan_room_release_notes.py`](scripts/plan_room_release_notes.py) + the planner contract tests and asserts no release side-effect file is produced. Added [`tests/test_release_dry_run_mode.py`](tests/test_release_dry_run_mode.py) locking in: safe default, read-only dry-run job, no publish step in dry-run, publish (`release`) job stays gated to `github.event_name == 'release'`, `softprops/action-gh-release` only in the `release` job, and dry-run plans the two release-eligible builds while excluding FanRelay / FanPWM / FanDAC. Updated [`docs/room-firmware-release-notes.md`](docs/room-firmware-release-notes.md), [`docs/room-firmware-release-matrix.md`](docs/room-firmware-release-matrix.md), [`docs/release-artifact-readiness-matrix.md`](docs/release-artifact-readiness-matrix.md), and this `UPCOMING_PR.md`. Validation: `python3 scripts/plan_room_release_notes.py`; `python3 tests/test_plan_room_release_notes.py`; `python3 tests/test_release_dry_run_mode.py`; `python3 tests/validate_configs.py`; `python3 scripts/validate_compile_targets.py --metadata-only`; `python3 tests/validate_webflash_builds.py`; `python3 tests/test_workflow_permissions.py`; `python3 -m unittest discover -s tests -p "test_*.py"` (883 tests, 3 skipped). | **Workflow + test + docs only.** The `release` (publish) job gate is **unchanged** (`if: github.event_name == 'release'`); no new publish/upload step; no `softprops/action-gh-release` added outside the existing `release` job; the dry-run job grants no `contents: write`. No GitHub Release published; no `.bin` / checksum / build-info committed; no `products/**` or `products/webflash/**` edit; no fan `artifact_name`; no `webflash_build_matrix` flip; no `firmware/sources.json` / `manifest.json` write; no `config/**` or product/package YAML change. FanRelay / FanPWM / FanDAC stay excluded (manual-candidate-only); FanTRIAC stays `blocked` / `HW-005`. Release-One stays `Ceiling-POE-VentIQ-RoomIQ` / `stable`; LED preview stays `Ceiling-POE-VentIQ-RoomIQ-LED` / `preview`. **No claim of fan release / WebFlash / hardware-stable / compliance readiness.** | The release workflow can now be exercised as a non-publishing dry-run on manual dispatch; publishing remains gated to real release events. No queue-ordering effect on the blocked fan release slices. |
...
| BOARD-PACKAGE-LAYER-002 | (this PR) | esphome-public | Merged — remaining first-cut board packages (RoomIQ, AirIQ, VentIQ, LED, PoE PSU) | Completed the first-cut `packages/boards/` layer (Core + these five) by authoring the five remaining SELV / low-voltage SKU board packages plus mount/variant overlays, each a thin `!include` wrapper over the existing functional source package(s) so behaviour is byte-identical, mirroring the Core pattern landed by `BOARD-PACKAGE-LAYER-001`: [`packages/boards/s360-200-roomiq.yaml`](packages/boards/s360-200-roomiq.yaml) (+ `s360-200-roomiq-wall.yaml`), [`packages/boards/s360-210-airiq.yaml`](packages/boards/s360-210-airiq.yaml) (+ `s360-210-airiq-wall.yaml`, `s360-210-airiq-ceiling-s3.yaml`), [`packages/boards/s360-211-ventiq.yaml`](packages/boards/s360-211-ventiq.yaml) (+ `s360-211-ventiq-pro.yaml`), [`packages/boards/s360-300-led.yaml`](packages/boards/s360-300-led.yaml) (+ `s360-300-led-wall.yaml`, `s360-300-led-mic-ceiling.yaml`, `s360-300-led-mic-wall.yaml` voice variants), and [`packages/boards/s360-410-poe-psu.yaml`](packages/boards/s360-410-poe-psu.yaml). `ceiling_halo_leds.yaml` stays a feature (cross-referenced by the LED board package, not absorbed). Legacy expansion/hardware paths preserved unchanged as source-of-truth aliases (§3.3) — **not** rewired to one-line includes (the consumer-side flip + test repoint is deferred to a later `PACKAGE-RENAME` slice / `CI-REFACTOR-VERIFY-001` per §5.5, because `tests/test_core_abstract_bus.py` and `tests/test_led_package_mapping.py` assert on the self-contained text of the folded functional packages). Updated [`docs/arch-board-bundle-plan.md`](docs/arch-board-bundle-plan.md) (first-cut board layer marked complete) and this `UPCOMING_PR.md`. Validation: `python3 tests/validate_configs.py` (212 files); `python3 scripts/validate_compile_targets.py --metadata-only`; `python3 tests/test_core_abstract_bus.py`; `python3 tests/test_product_substitutions.py`; `python3 tests/test_product_catalog.py`; `python3 tests/test_all_yaml_release_matrix.py`; `python3 tests/test_led_package_mapping.py`; `python3 -m unittest discover -s tests -p "test_*.py"` (1154 tests, 3 skipped). | **Additive board layer + docs only.** No product / config-string / artifact / workflow / WebFlash change; no `products/**`, `products/webflash/**`, `config/*.json`, `manifest.json`, or `firmware/sources.json` edit; no legacy package path renamed / moved / deleted (aliases only, paths preserved); mains boards `S360-310` / `S360-320` / `S360-400` NOT promoted into `packages/boards/`; no `schematic_status` / lifecycle change; LED stays preview-gated; `S360-410` stays `cataloged_unverified` (no PoE evidence claimed); `S360-311` / `S360-312` stay expansions; no test weakened. Release-One stays `Ceiling-POE-VentIQ-RoomIQ` / `stable`; LED preview stays `Ceiling-POE-VentIQ-RoomIQ-LED` / `preview`. Full `esphome compile` is **pending-ci** (ESPHome not available; no compile result claimed). | First-cut board layer is now complete (Core + RoomIQ + AirIQ + VentIQ + LED + PoE PSU). **Next phase: `BUNDLE-LAYER-001`** (assemble the product-named bundle YAMLs from boards + expansions + base + profiles, with customer-include compat shims). Mains boards `S360-310` / `S360-320` / `S360-400` board-layer promotion remains deferred (compliance-gated). |
| BUNDLE-LAYER-001 | (this PR) | esphome-public | Merged — bundle layer + compat shims for the two WebFlash-shipping config strings | Introduced `products/bundles/` and authored the two config-string-named bundle YAMLs for the live WebFlash builds — [`products/bundles/ceiling-poe-ventiq-roomiq.yaml`](products/bundles/ceiling-poe-ventiq-roomiq.yaml) (production / stable) and [`products/bundles/ceiling-poe-ventiq-roomiq-led.yaml`](products/bundles/ceiling-poe-ventiq-roomiq-led.yaml) (preview) — each assembling the docs/arch-board-bundle-plan.md §2.2 board stack from `packages/boards/` (`s360-100-core-ceiling` + `s360-410-poe-psu` + `s360-211-ventiq` + `s360-200-roomiq` [+ `s360-300-led` for the preview]) plus the base tier and the feature profiles, carrying the same substitutions, entity names, config string, and artifact-name identity as the product they replace. Converted the two shipping products [`products/sense360-ceiling-poe-ventiq-roomiq.yaml`](products/sense360-ceiling-poe-ventiq-roomiq.yaml) and [`products/sense360-ceiling-poe-ventiq-roomiq-led.yaml`](products/sense360-ceiling-poe-ventiq-roomiq-led.yaml) into thin compat shims (§3.2) that do nothing but `!include` their bundle, paths preserved. Resolution proven identical offline (leaf functional packages, merged substitutions, and `text_sensor` identity byte-equal between the pre-shim product and the shim→bundle→board chain). **Scoped to the shipping pair:** the five non-shipping config strings (`Ceiling-POE-VentIQ-FanRelay-RoomIQ`, `Ceiling-POE-VentIQ-FanTRIAC-RoomIQ`, `Ceiling-POE-FanDAC`, `Ceiling-POE-FanPWM`, `Ceiling-POE-RoomIQ`) are **deferred to BUNDLE-LAYER-002 (item 50a)** because their package-/product-layer guardrail tests (`tests/test_fandac_package.py`, `tests/test_fan_relay_package.py`, `tests/test_{relay,pwm,dac}_product_readiness.py`) assert on the product-internal content the bundle move relocates, and §5.5 requires those test repoints to travel in the same slice as the move — so the conversion + test edits are pulled into BUNDLE-LAYER-002 rather than weakening tests here. Updated [`docs/arch-board-bundle-plan.md`](docs/arch-board-bundle-plan.md) (§2.2 + §7 item 3 marked done / scoped) and this `UPCOMING_PR.md`. Validation: `python3 tests/validate_configs.py` (214 files); `python3 scripts/validate_compile_targets.py --metadata-only`; `python3 tests/test_release_one_entity_names.py`; `python3 tests/test_product_substitutions.py`; `python3 tests/test_product_catalog.py`; `python3 tests/validate_webflash_builds.py`; `python3 tests/test_webflash_artifact_naming.py`; `python3 tests/test_all_yaml_release_matrix.py`; `python3 -m unittest discover -s tests -p "test_*.py"` (1154 tests, 3 skipped). | **Bundle layer + shim conversions only.** Config strings, artifact names, `config/webflash-builds.json`, `manifest.json`, `firmware/sources.json` **byte-identical**; no workflow / `config/*.json` / test edit; no product lifecycle or `schematic_status` change; LED not marked stable; `S360-410` not verified; no fan blocker closed; FanTRIAC stays `blocked` / `HW-005`; customer include paths preserved; the `products/webflash/* → products/sense360-<stem>.yaml → bundle` chain still resolves to real files; the WebFlash repo (`sense360store/WebFlash`) is untouched. Full `esphome compile` is **pending-ci** (ESPHome not available; no compile result claimed). | Bundle layer landed for the two WebFlash-shipping config strings (the production + preview proof targets). **Next phase: `BUNDLE-LAYER-002`** (remaining five bundles + shim conversions + the §5.5 test repoints), then `PACKAGE-RENAME-00x` and `CI-REFACTOR-VERIFY-001`. |
| BUNDLE-LAYER-002 | (this PR) | esphome-public | Merged — bundle + shim the five remaining config strings; repoint structural tests | Authored the five remaining config-string bundles under [`products/bundles/`](products/bundles/) — [`ceiling-poe-roomiq.yaml`](products/bundles/ceiling-poe-roomiq.yaml), [`ceiling-poe-fanpwm.yaml`](products/bundles/ceiling-poe-fanpwm.yaml), [`ceiling-poe-fandac.yaml`](products/bundles/ceiling-poe-fandac.yaml), [`ceiling-poe-ventiq-fanrelay-roomiq.yaml`](products/bundles/ceiling-poe-ventiq-fanrelay-roomiq.yaml), [`ceiling-poe-ventiq-fantriac-roomiq.yaml`](products/bundles/ceiling-poe-ventiq-fantriac-roomiq.yaml) — each carrying the moved product composition **verbatim** (same substitutions, entity names, config string, artifact-name identity, and header wording; include depth `../packages` → `../../packages`), and converted the five `products/sense360-*.yaml` config-string products into thin compat shims (§3.2), paths preserved. Repointed the structural / fan-readiness tests that read the now-moved product-internal layout onto the bundle files — `tests/test_pwm_product_readiness.py`, `tests/test_dac_product_readiness.py`, `tests/test_relay_product_readiness.py`, `tests/test_native_fan_gpio_map.py`, `tests/test_fan_pwm_package.py`, `tests/test_fan_relay_package.py`, `tests/test_fandac_package.py`, `tests/test_compile_targets.py` — assertions preserved, only the path read follows the moved content; catalog / compile-only-target path-identity references stay on the product shim. Marked the bundle layer complete in [`docs/arch-board-bundle-plan.md`](docs/arch-board-bundle-plan.md) §7 item 3a and updated this `UPCOMING_PR.md`. Validation: `python3 tests/validate_configs.py`; `python3 scripts/validate_compile_targets.py --metadata-only`; `python3 tests/test_release_one_entity_names.py`; `python3 tests/test_product_substitutions.py`; `python3 tests/test_pwm_product_readiness.py`; `python3 tests/test_native_fanpwm_yaml.py`; `python3 tests/test_product_catalog.py`; `python3 tests/validate_webflash_builds.py`; `python3 tests/test_webflash_artifact_naming.py`; `python3 -m unittest discover -s tests -p "test_*.py"` (1154 tests, 3 skipped). | **Bundle + shim + structural-test repoint only.** Config strings, artifact names, `config/webflash-builds.json`, `manifest.json`, `firmware/sources.json` **byte-identical**; no workflow / `config/*.json` edit; the two shipping shims + bundles from BUNDLE-LAYER-001 untouched; `tests/test_release_one_entity_names.py` and `tests/test_product_substitutions.py` untouched (no shipping-identity assertion weakened); no product lifecycle / `schematic_status` change; LED not marked stable; `S360-410` not verified; no fan blocker closed; FanTRIAC stays `blocked` / `HW-005`; customer include paths preserved; WebFlash repo untouched. Full `esphome compile` is **pending-ci** (ESPHome unavailable; no compile result claimed). | The bundle layer is **complete** — all seven config strings (two shipping + five non-shipping) are bundled + shimmed. **Next phase: `PACKAGE-RENAME-00x`** (rebind consumers onto the `packages/boards/` SKU packages, aliasing legacy names), then `CI-REFACTOR-VERIFY-001` (gate-parity proof). |
...
| CI-REFACTOR-VERIFY-001 | (this PR) | esphome-public | Merged — CI / gate parity proof + `docs/ci-pipeline.md` refresh (no workflow edit) | Proved the CI workflows and PR gates behave identically after the board-package / bundle / rename refactor by running the actual sweep commands against the post-refactor layout and showing each passes as-is: `yamllint -c .yamllint products/ packages/` → exit 0 / clean (recurses into `packages/boards/` + `products/bundles/`; boards, bundles, aliases, shims all clean); the `find products/ -name "*.yaml" -type f ! -name "secrets.yaml" ! -path "products/webflash/*"` discovery → 43 files (7 `products/bundles/*` + 28 top-level shims/legacy + 8 `products/compile-only/*`), bundles auto-discovered, `products/webflash/*` still excluded, and every discovered file's `!include` chain resolves (shim → bundle → board package → base/feature tiers, verified by a recursive include-graph walk); the `sed` targets intact (`packages/base/external_components.yaml` still carries the `ref: main` literal; `packages/features/` still holds the 30 `@main` `find … -exec sed` YAMLs — base/feature tiers untouched, and bundles reach `external_components` via `!include ../../packages/base/external_components.yaml`); `python3 scripts/validate_compile_targets.py --metadata-only` → 14 targets, every `product_yaml` resolves through the shims/bundles; `python3 tests/validate_configs.py` (223 files / 0 failed); `python3 tests/test_release_one_entity_names.py`; `python3 tests/test_product_substitutions.py` (28 products); `python3 -m unittest discover -s tests -p "test_*.py"` (1154 tests, 3 skipped) plus `validate_webflash_builds.py` / `test_webflash_artifact_naming.py` / `test_product_catalog.py` / `validate_product_catalog_consistency.py`. Confirmed gate identity: `validate.yml` (`name: Quick Validation`) and `firmware-build-release.yml` (`name: Build & Release Firmware`) `name:` fields unchanged — `git diff` vs `main` shows the whole `.github/workflows/` tree byte-identical — so no branch-protection required-status-check name moved. Updated [`docs/ci-pipeline.md`](docs/ci-pipeline.md) (new boards/bundles/aliases/shims layout section + per-workflow handling + gate-identity table; corrected the stale `find products/` build-matrix wording to config-string-driven) and [`docs/arch-board-bundle-plan.md`](docs/arch-board-bundle-plan.md) §7 item 5 (done + evidence) and this `UPCOMING_PR.md`. | **Parity proof + docs only.** No workflow trigger / step / gate-logic edit and **no workflow `name:` field touched** (`.github/workflows/` byte-identical to `main`); no glob fix applied (none required with bundles under `products/`, per §5 — the §5.5 test repoints already travelled with `PACKAGE-RENAME-001..005`); no product / package / board / bundle / alias YAML change; no config-string / artifact / lifecycle / `schematic_status` change; `config/webflash-builds.json` / `manifest.json` / `firmware/sources.json` untouched; WebFlash repo untouched. Full `esphome compile` is **pending-ci** (ESPHome not available; metadata/structure parity shown, no compile result claimed). | CI / gate parity is proven across the refactor and `docs/ci-pipeline.md` now documents the boards/bundles/aliases/shims layout. **Next: `DOCS-ARCH-REFRESH-001`** (the remaining §6.1 doc set), then `WEBFLASH-ARCH-SYNC-001` (WebFlash repo). |
...
30. **WEBFLASH-DAC-001-READINESS — Re-evaluate DAC WebFlash readiness**
    - Status: **DONE — docs-only (2026-05-26; this PR); no exposure flip**
    - Purpose: Re-evaluate whether FanDAC / S360-312 is ready for a future
      WebFlash-exposure PR, using the latest package / product / full-compile
      and `WEBFLASH-DRIFT-001` / PR #595 drift-audit evidence, **without
      exposing it to WebFlash**. Added a re-evaluated **DAC WebFlash
      readiness table** (gate / status / evidence / next action) to
      [`docs/webflash-exposure-readiness-matrix.md`](docs/webflash-exposure-readiness-matrix.md)
      §DAC / S360-312 WebFlash posture, plus audit-log / pointer notes in
      [`docs/release-artifact-readiness-matrix.md`](docs/release-artifact-readiness-matrix.md),
      [`docs/product-readiness-matrix.md`](docs/product-readiness-matrix.md),
      and [`docs/webflash-drift-audit.md`](docs/webflash-drift-audit.md) (§4.2
      follow-up resolution).
    - Findings: **WebFlash DAC exposure stays blocked.** `CLOSED` — product
      YAML, package layer, full-compile evidence (run `26364679370`,
      prior-recorded), `compile_validation_status: validated-full-compile`,
      `voltage_enum_fixed`, package/product config-posture hardware evidence,
      and the `FanDAC ↔ AirIQ` mutex (0 AirIQ-bearing FanDAC combos). This is
      **stronger** than FanRelay — DAC's compile-status flag is already set,
      so there is no compile-flag gap (FanRelay drift row #21) to resolve.
      `BLOCKING` — WebFlash wrapper, `config/webflash-builds.json` row,
      `artifact_name`, firmware release artifact, import-source path,
      voltage-mode / product UX, compliance / safety. `NEEDS-TOOLING` — live
      `sense360store/WebFlash` re-verification and the still-owed `S360-312`
      module-availability classification (`WEBFLASH-DRIFT-001` row #17; read
      access denied this session). `NEEDS-OPERATOR-INPUT` — the `J3` `out0`/
      `out1` silkscreen transposition confirmation and the Cloudlift S12
      harness / product-bench caveat. `OUT-OF-SCOPE` — REQUIRED_CONFIGS / kit /
      recommended membership (never by default; `S360-KIT-DUCT-0-10V` stays
      future-expansion / hardware-pending).
    - Recommends next: **`WEBFLASH-DAC-LIVE-CHECK-001`** (re-run the live
      WebFlash readiness / drift check once read access is restored; record
      the `S360-312` module-availability classification) — **not**
      `WEBFLASH-DAC-002-WRAPPER-PLAN` (non-WebFlash gates are not all clean)
      and **not** `WEBFLASH-DAC-001`.
    - Guardrails: no `products/**`, `products/webflash/**`,
      `config/webflash-builds.json`, any other `config/**`, `firmware/**`,
      `manifest.json`, `firmware/sources.json`, `tests/**`, workflow,
      component, include, or WebFlash-repo edit; no wrapper, no
      `webflash_build_matrix` flip, no `artifact_name`, no release artifact, no
      WebFlash exposure; the `FanDAC ↔ AirIQ` mutex is not relaxed; no import /
      release / compliance / hardware-stable / security clean-bill claim; no
      fabricated WebFlash evidence. Release-One + LED preview + FanTRIAC
      (`blocked` / `HW-005`) unchanged; `S360-312` stays `cataloged_unverified`.
31. **WEBFLASH-PWM-001-READINESS — Re-evaluate PWM WebFlash readiness**
    - Status: **DONE — docs-only (2026-05-26; this PR); no exposure flip**
    - Purpose: Re-evaluate whether FanPWM / S360-311 is ready for a future
      WebFlash-exposure PR, using the latest package / product / full-compile /
      product-validation and `WEBFLASH-DRIFT-001` / PR #595 drift-audit
      evidence, **without exposing it to WebFlash**. Added a re-evaluated
      **PWM WebFlash readiness table** (gate / status / evidence / next action)
      to [`docs/webflash-exposure-readiness-matrix.md`](docs/webflash-exposure-readiness-matrix.md)
      §PWM / S360-311 WebFlash posture, plus audit-log / pointer notes in
      [`docs/release-artifact-readiness-matrix.md`](docs/release-artifact-readiness-matrix.md),
      [`docs/product-readiness-matrix.md`](docs/product-readiness-matrix.md),
      and [`docs/webflash-drift-audit.md`](docs/webflash-drift-audit.md) (§4.3
      follow-up resolution).
    - Findings: **WebFlash PWM exposure stays blocked.** `CLOSED` — product
      YAML ([`products/sense360-ceiling-poe-fanpwm.yaml`](products/sense360-ceiling-poe-fanpwm.yaml),
      PR #593), package layer (PACKAGE-PWM-001-IMPLEMENT-001 / PR #590,
      PWM-drive-only), full-compile evidence (run `26414398902`,
      prior-recorded), `compile_validation_status: validated-full-compile`,
      `rpm_supported: false`, product-YAML-composition parity
      (FW-COMPILE-PWM-PRODUCT-001 / PR #594), and package/product
      config-posture hardware evidence. This is **stronger** than FanRelay —
      PWM's compile-status flag is already set, so there is no compile-flag
      gap (FanRelay drift row #21) to resolve. `BLOCKING` — WebFlash wrapper,
      `config/webflash-builds.json` row, `artifact_name`, firmware release
      artifact, import-source path, no-RPM / product UX. `NEEDS-TOOLING` —
      live `sense360store/WebFlash` re-verification, the still-owed `S360-311`
      module-availability classification (`WEBFLASH-DRIFT-001` row #16; read
      access denied this session), and the live product-YAML `esphome config`
      (ESPHome not present this session; byte-equivalent composition
      full-compiled via the compile-only skeleton). `NEEDS-OPERATOR-INPUT` —
      **PWM polarity** bench, **per-fan / aggregate current + thermal
      envelope**, **product bench**, `TachIO` / `GPIO16` reserved/pending, and
      board-level thermal / EMI (the board is SELV — no mains — so the
      `COMPLIANCE-001` mains gate does not apply to FanPWM). `OUT-OF-SCOPE` —
      REQUIRED_CONFIGS / kit / recommended membership (never by default;
      legacy four-channel `sense360-fan-pwm.yaml` retention / migration /
      removal owned by `PRODUCT-PWM-001`).
    - Recommends next: **`WEBFLASH-PWM-LIVE-CHECK-001`** (re-run the live
      WebFlash readiness / drift check once read access is restored; record
      the `S360-311` module-availability classification), with
      **`S360-311-BENCH-001`** (PWM polarity + per-fan / aggregate current +
      thermal envelope + product bench) as the substantive evidence gate —
      **not** `WEBFLASH-PWM-002-WRAPPER-PLAN` (non-WebFlash gates are not all
      clean) and **not** `WEBFLASH-PWM-001`.
    - Guardrails: no `products/**`, `products/webflash/**`,
      `config/webflash-builds.json`, any other `config/**`, `firmware/**`,
      `manifest.json`, `firmware/sources.json`, `tests/**`, workflow,
      component, include, or WebFlash-repo edit; no wrapper, no
      `webflash_build_matrix` flip, no `artifact_name`, no release artifact, no
      WebFlash exposure; no RPM added or claimed; no import / release /
...
37. **S360-312-BENCH-EVIDENCE-REQUEST-001 — Define FanDAC bench evidence
    ...
    - Guardrails: no `packages/**`, `products/**`, `products/webflash/**`,
      ...
      FanTRIAC (`blocked` / `HW-005`) unchanged.
38. **S360-311-CURRENT-THERMAL-EVIDENCE-REQUEST-001 — Define FanPWM current
    & thermal evidence checklist**
    - Status: **DONE — docs-only (2026-05-27; this PR); no exposure flip,
      no behaviour change**
    - Purpose: Turn the still-open **measured** FanPWM rows (`PWM-6`
      per-channel + aggregate current, `PWM-13` thermal) — scoped by
      `PWM-BLOCKER-RECLASSIFY-001` (PR #602) as gating **only** WebFlash /
      release / hardware-stable / electrical-margin, not repo / package /
      product / config work — into a precise operator-answerable checklist
      and pass/fail evidence contract so a later `S360-311-CURRENT-THERMAL-001`
      is small and evidence-backed. Canonical checklist + contract in
      [`docs/hardware/s360-311-r4-pwm.md`](docs/hardware/s360-311-r4-pwm.md)
      §S360-311-CURRENT-THERMAL-EVIDENCE-REQUEST-001; cross-lane copy in
      [`docs/blocker-burndown.md`](docs/blocker-burndown.md) §5D + §4
      pointer; dated addendum in
      [`docs/product-readiness-matrix.md`](docs/product-readiness-matrix.md)
      §FanPWM / S360-311.
    - Checklist (operator-answerable): board revision tested; fan model +
      label current rating; supply voltage + **measured supply current**;
      **per-channel current at full speed**; **aggregate current with all
      four channels active**; **inrush / startup current** if measurable;
      **MT3608 input / output current or measured ceiling**; **thermal
      method** (IR thermometer / thermal camera / thermocouple / qualitative
      only); **test duration**; **ambient temperature**; **hottest component
      / location**; **measured max temperature**; whether all four channels
      ran at high / full duty; whether any **voltage drop, instability, or
      reset** occurred; **`TachIO`/`GPIO16` observation only if tested — no
      RPM claim unless separately proven**; photos / videos / logs if
      available; operator / date / source / provenance.
    - Evidence contract: defines what PASS-grade evidence closes per-channel
      current (`PWM-6`), aggregate current (`PWM-6`), MT3608 ceiling
      (`PWM-6`), electrical-margin observation (`PWM-6`), and measured
      thermal (`PWM-13`), and what stays out-of-scope — RPM support, WebFlash
      exposure, release artifact, import readiness, hardware-stable
      promotion, and compliance approval.
    - Drive re-search (this session): searched `S360-311` / `S360-311-R4` /
      `FanPWM` / `PWM` / `current` / `thermal` / `inrush` / `MT3608` /
      `bench` / `fan` and spreadsheets / PDFs / photos — **no measurement
      artifact found**, only design / CAD material (the
      `12vFan_PWM_PulseCounter` set incl. `PWM.png`, the canonically-named
      `S360-311-R4` Drive folder owned by `kanyugistash@gmail.com` created
      2026-05-16 with KiCad sources / gerbers / CPL / STEP / BOM / schematic
      PDF / renders, and the unchanged `Sense360_R4_Tracker`), recorded for
      provenance only and committing no Drive file.
    - Recommends next: **`S360-311-CURRENT-THERMAL-001`** — gated until the
      operator supplies the measured rows; no measured-current / thermal row
      can close in a docs-only pass. WebFlash wrapper / build PRs
      (`WEBFLASH-PWM-001` / `RELEASE-PWM-001` / `WF-IMPORT-PWM-001`) stay
      blocked; `WEBFLASH-PWM-LIVE-CHECK-001` stays queued behind WebFlash
      access restoration.
    - Guardrails: no `packages/**`, `products/**`, `products/webflash/**`,
...
49a. **BOARD-PACKAGE-LAYER-002 — Add the remaining first-cut board packages
    into `packages/boards/`**
    - Status: **Done — first-cut board layer complete (2026-05-30).** This
      slice ships the **five remaining first-cut SELV / low-voltage board
      packages**, each a thin `!include` wrapper over the existing functional
      package(s) so behaviour is byte-identical, mirroring the Core pattern
      landed by BOARD-PACKAGE-LAYER-001:
      `packages/boards/s360-200-roomiq.yaml` (+ `s360-200-roomiq-wall.yaml`
      mount overlay), `s360-210-airiq.yaml` (+ `s360-210-airiq-wall.yaml`,
      `s360-210-airiq-ceiling-s3.yaml`), `s360-211-ventiq.yaml`
      (+ `s360-211-ventiq-pro.yaml`), `s360-300-led.yaml`
      (+ `s360-300-led-wall.yaml`, `s360-300-led-mic-ceiling.yaml`,
      `s360-300-led-mic-wall.yaml` voice variants), and `s360-410-poe-psu.yaml`.
      `ceiling_halo_leds.yaml` stays a feature (cross-referenced by the LED
      board package, not absorbed). The legacy expansion/hardware paths are
      preserved unchanged as source-of-truth aliases (§3.3) and are **not**
      rewired to one-line includes — the consumer-side flip + test repoint is
      deferred to a later `PACKAGE-RENAME` slice / `CI-REFACTOR-VERIFY-001`
      (§5.5), because `tests/test_core_abstract_bus.py` and
      `tests/test_led_package_mapping.py` assert on the self-contained text of
      the folded functional packages. The production
      (`Ceiling-POE-VentIQ-RoomIQ`) and LED-preview
      (`Ceiling-POE-VentIQ-RoomIQ-LED`) products still resolve and validate
      unchanged; metadata validators pass; full `esphome compile` is
      **pending-ci** (ESPHome not available in this environment). **The mains
      boards `S360-310`/`S360-320`/`S360-400` remain OUT of the board layer for
      now** (compliance-gated; deferred to a later slice — they are NOT promoted
      by this slice per its guardrails).
    - Guardrails: WebFlash contract untouched (no config string, artifact,
      `webflash-builds.json`, `manifest.json`, or `firmware/sources.json`
      change); no product rebinds; legacy packages all still present; no
      workflow, schematic_status, or lifecycle change; LED stays preview-gated;
      `S360-410` stays `cataloged_unverified` (no PoE evidence claimed);
      `S360-311`/`S360-312` stay expansions until `HW-PINMAP-311/312-FOLLOWUP`
      evidence closes; no test weakened (any path-constant test repoint travels
      with CI-REFACTOR-VERIFY-001 / §5.5).
    - Dependency: `BOARD-PACKAGE-LAYER-001`. **Next phase: BUNDLE-LAYER-001.**

50. **BUNDLE-LAYER-001 — Introduce the bundle layer + customer-include compat
    shims (WebFlash-shipping pair)**
    - Status: **DONE — this PR (2026-05-30), scoped to the two WebFlash-shipping
      config strings.** Introduced `products/bundles/` and authored the two
      config-string-named bundles for the live WebFlash builds —
      [`products/bundles/ceiling-poe-ventiq-roomiq.yaml`](products/bundles/ceiling-poe-ventiq-roomiq.yaml)
      (production / stable) and
      [`products/bundles/ceiling-poe-ventiq-roomiq-led.yaml`](products/bundles/ceiling-poe-ventiq-roomiq-led.yaml)
      (preview) — each assembling the §2.2 board stack from `packages/boards/`
      (`s360-100-core-ceiling` + `s360-410-poe-psu` + `s360-211-ventiq` +
      `s360-200-roomiq` [+ `s360-300-led` for the preview]) plus the base tier
      and the feature profiles, carrying the same substitutions, entity names,
      config string, and artifact-name identity as the product they replace.
      Converted the two shipping products
      ...
      into thin compat shims that do nothing but `!include` their bundle (§3.2),
      paths preserved. Identical resolution proven offline (leaf functional
      packages, merged substitutions, and `text_sensor` identity all byte-equal
      between the pre-shim product and the shim→bundle→board chain); full
      `esphome compile` is **pending-ci** (ESPHome not available — no compile
      result claimed).
    - **Scope decision (recorded, not silently resolved):** the remaining **five
      non-shipping config strings** (`Ceiling-POE-VentIQ-FanRelay-RoomIQ`,
      `Ceiling-POE-VentIQ-FanTRIAC-RoomIQ`, `Ceiling-POE-FanDAC`,
      `Ceiling-POE-FanPWM`, `Ceiling-POE-RoomIQ`) are **deferred to
      BUNDLE-LAYER-002** (item 50a). Reason: authoring their bundles and/or
      shimming their products breaks the package-/product-layer guardrail tests
      that assert on the now-moved product-internal content
      ...
      product YAML under `products/`", and the
      `tests/test_{relay,pwm,dac}_product_readiness.py` substitutions / include /
      header-wording assertions). Per §5.5 those tests must travel in the same
      slice that moves the content they assert on, so the fan/blocked/roomiq
      conversions + their test repoints are pulled into BUNDLE-LAYER-002 rather
      than landed here. The two shipping products have no such content-asserting
      tests, so they convert cleanly with **no test edit** in this slice.
    - Guardrails honoured: config strings, artifact names,
      `config/webflash-builds.json`, `manifest.json`, `firmware/sources.json`
      byte-identical; no workflow / `config/*.json` / test edit; no lifecycle or
      `schematic_status` change; LED not marked stable; `S360-410` not verified;
      no fan blocker closed; customer include paths preserved; the
      `products/webflash/* → products/sense360-<stem>.yaml → bundle` resolution
      still hits real files; WebFlash repo untouched.
    - Validation: `python3 tests/validate_configs.py` (214 files);
      `python3 scripts/validate_compile_targets.py --metadata-only`;
      ...
      `python3 tests/test_all_yaml_release_matrix.py`;
      `python3 -m unittest discover -s tests -p "test_*.py"` (1154 tests, 3
      skipped).
    - Dependency: `BOARD-PACKAGE-LAYER-001` / `-002`.

50a. **BUNDLE-LAYER-002 — Bundle the five non-shipping config strings + shim
     conversions (with the test repoints)**
    - Status: **DONE (this PR, 2026-05-30).** All five non-shipping bundles +
      shims landed and the §5.5 structural / fan-readiness tests are repointed
      to the bundle files; the bundle layer is complete (all seven config
      strings bundled + shimmed). Next phase: PACKAGE-RENAME-00x, then
      CI-REFACTOR-VERIFY-001.
    - Purpose: Author the remaining five config-string bundles under
      `products/bundles/` (`ceiling-poe-ventiq-fanrelay-roomiq`,
      `ceiling-poe-ventiq-fantriac-roomiq`, `ceiling-poe-fandac`,
      `ceiling-poe-fanpwm`, `ceiling-poe-roomiq`), each assembling boards +
      expansions (the fan-driver SKUs `S360-310`/`320`/`311`/`312` stay
      expansions, not board packages) + base + profiles, and convert each
      remaining `products/sense360-*.yaml` config-string product into its thin
      compat shim. Repoint the content-asserting tests
      (`tests/test_fandac_package.py`, `tests/test_fan_relay_package.py`,
      `tests/test_{relay,pwm,dac}_product_readiness.py`) onto the bundle (the new
      home of the moved includes / substitutions / header wording) **in the same
      slice** per §5.5.
    - Guardrails: same as BUNDLE-LAYER-001 — config strings + artifact names +
      `config/webflash-builds.json` byte-identical; no lifecycle /
      `schematic_status` change; fan blockers stay blocked; FanTRIAC stays
      blocked under HW-005; resolution proven identical.
...
52. **CI-REFACTOR-VERIFY-001 — Confirm CI / gate parity**
    - **Status: DONE (this PR, 2026-05-30). Parity proven; NO workflow edit
      required.** Ran the actual CI sweep commands locally against the
      post-refactor layout and showed each passes as-is:
      `yamllint -c .yamllint products/ packages/` → exit 0, clean (recurses into
      `packages/boards/` + `products/bundles/`; boards, bundles, aliases, shims
      all lint clean); the `find products/ … ! -path "products/webflash/*"`
      discovery → 43 files (7 bundles + 28 top-level shims/legacy + 8
      compile-only), bundles auto-discovered, webflash still excluded, and every
      discovered file's `!include` chain resolves (shim → bundle → board → base/
      feature tiers, verified by a recursive include-graph walk); the `sed`
      targets intact (`packages/base/external_components.yaml` still carries
      `ref: main`; `packages/features/` still holds the 30 `@main` `find … -exec
      sed` YAMLs — base/feature tiers untouched by the refactor, and bundles
      reach `external_components` via `!include ../../packages/base/…`);
      `python3 scripts/validate_compile_targets.py --metadata-only` → 14
      targets, every `product_yaml` resolves through the shims/bundles;
      ...
      `python3 -m unittest discover -s tests -p "test_*.py"` → 1154 tests / 3
      skipped / OK (plus `validate_webflash_builds.py`,
      `test_webflash_artifact_naming.py`, `test_product_catalog.py`,
      `validate_product_catalog_consistency.py` all green). **Gate identity
      confirmed:** `validate.yml` (`name: Quick Validation`) and
      `firmware-build-release.yml` (`name: Build & Release Firmware`) `name:`
      fields unchanged — `git diff` vs `main` shows the entire
      `.github/workflows/` tree byte-identical — so no branch-protection
      required-status-check name moved. **No glob fix applied** (none required
      with bundles under `products/`, per §5); the §5.5 test repoints already
      travelled with RENAME-001..005, so no test update remained for this slice.
      Updated [`docs/ci-pipeline.md`](docs/ci-pipeline.md) (boards/bundles/
      aliases/shims layout + per-workflow handling + gate-identity table) and
      [`docs/arch-board-bundle-plan.md`](docs/arch-board-bundle-plan.md) §7 item
      5 (marked done with the parity evidence) and this `UPCOMING_PR.md`.
    - Guardrails (honoured): gate semantics unchanged; no new build exposed; no
      workflow `name:`/trigger/step edit; no product / package / board / bundle /
      alias / config-string / artifact / test change. Full `esphome compile` is
      **pending-ci** (ESPHome not available; metadata/structure parity shown, no
      compile result claimed).
    - Dependency: ran after `PACKAGE-RENAME-001..005` (all merged).
    - **Next: `DOCS-ARCH-REFRESH-001`** (item 53), then `WEBFLASH-ARCH-SYNC-001`
      (item 54, WebFlash repo).
...
- **WF-IMPORT-PWM-001** — WebFlash-side import of the PWM product,
  **blocked** behind upstream `RELEASE-PWM-001`. **2026-05-26 —
  ...
  could NOT be re-verified live** — a read of `sense360store/WebFlash`
  returned access denied (session GitHub scope is `esphome-public` +
  `esphome` only), so the WebFlash-side facts stay **prior-recorded, not
  re-confirmed this session** (`NEEDS-TOOLING`). In particular the
  `S360-311` classification in `scripts/utils/module-availability.js`
  remains **not recorded** (`WEBFLASH-DRIFT-001` row #16) and cannot be
  captured without live access. `WF-IMPORT-PWM-001` stays **blocked**
  behind `RELEASE-PWM-001`; the recommended next esphome-public step is
  `WEBFLASH-PWM-LIVE-CHECK-001` to close the live-verification axis and
...
- **WF-IMPORT-DAC-001** — WebFlash-side import of the DAC product,
  **blocked** behind upstream `RELEASE-DAC-001`. **2026-05-26 —
  ...
  `esphome` only), so the WebFlash-side facts stay **prior-recorded, not
  re-confirmed this session** (`NEEDS-TOOLING`). In particular the
  `S360-312` classification in `scripts/utils/module-availability.js`
  remains **not recorded** (`WEBFLASH-DRIFT-001` row #17) and cannot be
  captured without live access. `WF-IMPORT-DAC-001` stays **blocked**
...
- **2026-05-27 — `S360-311-CURRENT-THERMAL-EVIDENCE-REQUEST-001` Drive
  re-search: no measured FanPWM current / thermal evidence (design / CAD
  only; provenance recorded, nothing committed).** Searched Google Drive
  for `S360-311`, `S360-311-R4`, `FanPWM`, `PWM`, `current`, `thermal`,
  `inrush`, `MT3608`, `bench`, `fan`, and spreadsheets / PDFs / photos.
  **No measurement artifact exists** — no per-channel or aggregate current
  log, no measured supply current, no inrush / locked-rotor capture, no
  MT3608 output-current ceiling note, and no measured thermal / IR /
  thermocouple reading. The only FanPWM-bearing material is **design /
  CAD**: the already-recorded `12vFan_PWM_PulseCounter` set (schematic PDF,
  `.xlsx` BOM, gerbers, CPL, STEP, KiCad sources, renders incl. `PWM.png`);
  the canonically-named `S360-311-R4` Drive folder (owner
  `kanyugistash@gmail.com`, folder created 2026-05-16) holding
  ...
  `Sense360_R4_Tracker` (2026-05-18). This is the same artifact class as
  the committed
  [`docs/hardware/schematics/S360-311-R4.pdf`](docs/hardware/schematics/S360-311-R4.pdf),
  is recorded for **provenance only**, closes **no** measurement blocker,
  and **no Drive file is committed by this PR**. The still-open measured
  FanPWM rows (`PWM-6` per-channel + aggregate current, `PWM-13` thermal)
  are converted into the operator checklist + pass/fail evidence contract
  in
  [`docs/hardware/s360-311-r4-pwm.md`](docs/hardware/s360-311-r4-pwm.md)
  §S360-311-CURRENT-THERMAL-EVIDENCE-REQUEST-001 and
  [`docs/blocker-burndown.md`](docs/blocker-burndown.md) §5D.
  `S360-311-CURRENT-THERMAL-001` stays **gated** until the operator
  supplies the measured rows. No claim of measured FanPWM current /
  thermal, RPM support, WebFlash / import / release readiness, compliance
  clearance, or hardware-stable readiness; `S360-311` stays
