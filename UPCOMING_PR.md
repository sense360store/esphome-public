...
- **FW-COMPILE-RELAY-001** (this PR) adds a single FanRelay
...
8. **WEBFLASH-RELAY-001** (preceded by **WEBFLASH-RELAY-001-READINESS-REFRESH** / PR #565 / 2026-05-22, docs-only; **FW-COMPILE-RELAY-001** this PR / 2026-05-22 added compile-only validation)
    - Status: **`WEBFLASH-RELAY-001-READINESS-REFRESH` merged as
      PR #565 (2026-05-22, docs-only); `FW-COMPILE-RELAY-001`
      merged this PR (2026-05-22, compile-only target add);
      `WEBFLASH-RELAY-001` itself
      remains blocked** on (1) explicit `WEBFLASH-RELAY-001`
      ...
      default `preview-candidate`). The recommended next Relay
      chain PR is one of `WEBFLASH-RELAY-001` implementation plan /
      scaffold only (if allowed by the project lead),
      `RELEASE-RELAY-001` (blocked until artifact path exists), or
      `FW-COMPILE-RELAY-001` (if compile-only validation should
      happen first); none of those is immediate
      `WEBFLASH-RELAY-001` wrapper / catalog / build-matrix work.
    ...
    - Notes: Pairs with WebFlash-side `WF-IMPORT-RELAY-001`. The
      ...
      **outcome-first** — e.g. "Switched fan control" or "Relay
      ...
      `WEBFLASH-RELAY-001-READINESS-REFRESH` (this PR) re-evaluated
      WebFlash exposure post-PR-#564 and recorded that WebFlash
      Relay exposure remains blocked.
...
9. **RELEASE-RELAY-001**
    ...
    - Notes: The `CORE-ABSTRACT-BUS-001A` substitution-layer
      ...
      (PR #564); `WEBFLASH-RELAY-001-READINESS-REFRESH` (this PR,
      docs-only) recorded that `RELEASE-RELAY-001` remains
      blocked until at minimum the seven WebFlash gates clear and
      a Relay artifact path exists. Remaining gates inherit from
      `WEBFLASH-RELAY-001` implementation + product / compliance /
      release-readiness gates. **No `RELEASE-RELAY-001` unblock
      claim is made by `WEBFLASH-RELAY-001-READINESS-REFRESH`.**
