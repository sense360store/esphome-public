...
> **Note on new schematic uploads.** New schematic PDFs for `S360-100`,
> `S360-200`, `S360-210`, `S360-211`, and `S360-300` have been shared
> alongside this audit. **Ingesting that schematic evidence — committing
> the PDFs, updating
> [`config/hardware-catalog.json`](../config/hardware-catalog.json)
> `schematic_status` rows, refreshing the per-board pin/connector
> references under `docs/hardware/`, and reclassifying the relevant rows
> in
> [`docs/hardware/remaining-board-documentation-audit.md`](hardware/remaining-board-documentation-audit.md#decision-table)
> — is deferred to HW-007.** COMPAT-001 deliberately reads the repo as
> committed at the time of this PR. None of the decisions in this audit
> change when HW-007 lands; HW-007 may upgrade rows from
> `cataloged-unverified` / `partially-documented` to `documented` and may
> unblock the *evidence* gate on future preview / stable promotions, but
> promotion still also requires a catalog entry, a build-matrix entry,
> a release-notes draft, a build/release proof, and (for mains-voltage
> modules) a qualified compliance review per
> [`docs/product-onboarding.md`](product-onboarding.md). HW-007 does not
> by itself ship any new firmware, and it does not change FanTRIAC's
> HW-005 blocked status or LED's Release-One exclusion.
...
| `AirIQ` | Yes — `canonical_modules`. Mutually exclusive with `VentIQ` (`rules.airiq_and_ventiq_mutually_exclusive: true`). | Not in any current catalog entry. Legacy `sense360-core-*` YAMLs that include AirIQ-style sensors are `legacy-compatible`. | `S360-210` Sense360 AirIQ is `cataloged-unverified`, `not-needed-for-release-one`. | Not in Release-One. | **No.** | Keep. A future `AirIQ`-bearing config requires `S360-210` schematic + pin/connector doc, a new catalog entry (acknowledging AirIQ ↔ VentIQ mutual exclusion), and a build-matrix entry. `FanDAC` is additionally forbidden alongside `AirIQ` by `rules.fandac_conflicts_with_airiq` — that rule already enforces the bus conflict for any future `AirIQ`-bearing build entry. |
| `VentIQ` | Yes — `canonical_modules`. | Yes — Release-One `modules.air_quality = "VentIQ"` and `hardware.ventiq = "S360-211"`. | `S360-211` Sense360 VentIQ is `partially-documented` (Core-side J9 nets captured; module-side schematic pending). | In Release-One. | Yes (air-quality slot for the only production entry). | Keep. The "schematic verification pending" caveat must remain visible in the catalog `notes` and in [`docs/release-one-hardware-audit.md`](release-one-hardware-audit.md). Do not promote that caveat away in this audit. |
...
- **`LED` — excluded from Release-One by config-string policy.** The
  ...
  product with its own catalog entry, build-matrix entry, schematic
  evidence (`S360-300`), and reconciliation of the `GPIO14` (package)
  vs `IO38` (schematic) `LED_DATA` discrepancy.
