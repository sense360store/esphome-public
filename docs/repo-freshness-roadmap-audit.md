...
| `compile-only.yml` (full lane) | **workflow_dispatch `compile_mode=full` only** | `esphome compile` of all **12** registered targets (manual) — count moved 10 → 12 by `TOPLEVEL-FAN-COMPILE-TARGETS-001` (2026-05-27), which registered the top-level FanPWM / FanDAC product YAMLs alongside their skeletons. Historical runs: CI run `26414398902` (10 targets); earlier run `26364679370` (9 targets); local 2026-05-27 (ESPHome 2026.4.5, 10/10 `rc=0`, **no CI run id**) — see [`product-readiness-matrix.md` §FW-FULL-COMPILE-NOWEBFLASH-001](product-readiness-matrix.md#fw-full-compile-nowebflash-001--recorded-full-esphome-compile-evidence-for-the-compile-only-lane-2026-05-27). The **2 new top-level targets carry `compile_validation_status: pending-ci`** (no full compile recorded yet; owed to `FW-FULL-COMPILE-TOPLEVEL-FANS-001`). | nothing automatically on PR; the 2 new top-level targets' full compile not yet recorded |
