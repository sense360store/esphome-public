...
| `config/compile-only-targets.json` | `MANUAL-FIRMWARE-CANDIDATE-001` (2026-05-27) | **ACTIVE-STABLE** | FanDAC / FanPWM / FanRelay top-level targets all `validated-full-compile` (FanDAC/FanPWM via PR #616; FanRelay's previously-unset flag recorded by `MANUAL-FIRMWARE-CANDIDATE-001` from the committed PR #616 run, target #8). Current. |
| `docs/product-readiness-matrix.md`, `docs/webflash-exposure-readiness-matrix.md`, `docs/release-artifact-readiness-matrix.md` | `MANUAL-FIRMWARE-CANDIDATE-001` (2026-05-27) | **ACTIVE-STABLE** | Append-only logs; **headline/current posture verified current**. FanRelay / FanPWM / FanDAC now recorded as **manual / no-WebFlash firmware candidates** (validated-full-compile; `product-readiness-matrix.md` §MANUAL-FIRMWARE-CANDIDATE-001, `release-artifact-readiness-matrix.md` candidate-table update note). Candidate ≠ release; no `not-release-ready` cell changed. Historical `pending-ci` rows correctly superseded. |
...
| `compile-only.yml` (full lane) | **workflow_dispatch `compile_mode=full` only** | `esphome compile` of all **12** registered targets (manual) — count moved 10 → 12 by `TOPLEVEL-FAN-COMPILE-TARGETS-001` (2026-05-27), which registered the top-level FanPWM / FanDAC product YAMLs alongside their skeletons. Runs: CI run `26414398902` (10 targets); earlier run `26364679370` (9 targets); local 2026-05-27 (ESPHome 2026.4.5, 10/10 `rc=0`, **no CI run id**, `FW-FULL-COMPILE-NOWEBFLASH-001`); and local 2026-05-27 **12/12 `rc=0`** (ESPHome 2026.4.5, **no CI run id**, `FW-FULL-COMPILE-TOPLEVEL-FANS-001`) — see [`product-readiness-matrix.md` §FW-FULL-COMPILE-TOPLEVEL-FANS-001](product-readiness-matrix.md#fw-full-compile-toplevel-fans-001--recorded-full-esphome-compile-evidence-for-the-top-level-fanpwm--fandac-product-yamls-2026-05-27). The **2 top-level targets are now `compile_validation_status: validated-full-compile`** (full compile recorded `rc=0` by `FW-FULL-COMPILE-TOPLEVEL-FANS-001`, superseding their prior `pending-ci`). | nothing automatically on PR; no CI-side `workflow_dispatch` run id for the local full lanes |
