...
| [`packages/expansions/fan_gp8403.yaml`](../../packages/expansions/fan_gp8403.yaml) | `S360-312` Sense360 DAC | `FanDAC` — dual-channel GP8403 12-bit DAC over I²C; two `fan: speed` entities + per-channel voltage / speed templates | [`s360-312-r4-dac.md`](s360-312-r4-dac.md) (HW-PINMAP-312) **`partial — schematic evidence available; package reconciliation pending`**; module-side schematic PDF + curated artifact index committed under HW-ASSETS-003; standalone schematic+BOM reference doc landed at [`s360-312-r4-fandac.md`](s360-312-r4-fandac.md) on 2026-05-22 (HW-PINMAP-312-FOLLOWUP — records `IC1`/`IC2` `GP8403-TC50-EW` BOM-confirmed, `SW1`/`SW2` `219-3MSTR` 3-pole DIP BOM-confirmed, `V5V` of each DAC hardwired to `+12V` ⇒ range is firmware/register-driven only, no `5V`/`10V` jumper present in BOM, both DACs share the canonical `core_i2c` bus after PR #569); `PACKAGE-DAC-001-READINESS-REFRESH` pass (2026-05-22; same doc, [§PACKAGE-DAC-001 readiness refresh](s360-312-r4-fandac.md#package-dac-001-readiness-refresh)) recorded the 10-row readiness table and the verdict that `PACKAGE-DAC-001` is **not implementation-plannable yet** | `needs-package-reconciliation` + `bench-evidence-pending` | **Of the 10 FanDAC-specific blockers in [`s360-312-r4-fandac.md`](s360-312-r4-fandac.md#package-dac-001-readiness-refresh), rows 1 (shared-I²C-bus naming), 2 (GP8403 BOM identity), 5 (firmware/register-driven range mechanism), and 7 (per-channel range mixing constraint) are resolved or evidence-captured; row 9 (Nextion / UART0) is deferred out of `PACKAGE-DAC-001` scope; rows 3, 6, 8, and 10 still block.** Specifically still blocking: **row 3** — DIP-switch I²C address truth table for `IC1` (`SW1`) and `IC2` (`SW2`) (package allows only `0x58` / `0x59`, addresses `IC1` only); **row 6** — output-range policy decision (operator-requested firmware/product-selectable 0–5 V / 0–10 V; the active YAML's `${fan_dac_voltage_mode}` is chip-level only and a `${fan_dac_voltage_mode_2}` would be needed to bind `IC2`); **row 8** — `J2` / `J3` Cloudlift S12 silkscreen pin-1 identity + harness conductor trace; **row 10** — package YAML correctness (stale "(jumper selectable on hardware)" comment at file line 6 still uncorrected; decision among "one logical FanDAC, two channels" vs "two GP8403 devices" vs "four logical outputs" still owed). Core `J7` pin-1 `+5V` vs Module `J1` pin-1 `+3.3V` voltage-rail discrepancy is also outstanding (parallel `S360-100-BENCH-001` track). The post-PR-#569 active YAML inherits `${fan_dac_i2c_id}: core_i2c`; the header-comment block lines 13–18 was updated by PR #569 to reference `GPIO48` / `GPIO45`. `S360-312` JSON `schematic_status` stays `cataloged_unverified`. Subject to `FanDAC` ↔ `AirIQ` mutex in [`config/webflash-compatibility.json`](../../config/webflash-compatibility.json). | **No package edit.** HW-PINMAP-312-FOLLOWUP `s360-312-r4-fandac.md` standalone reference doc has landed (2026-05-22); `PACKAGE-DAC-001-READINESS-REFRESH` has landed (2026-05-22; same doc, [§PACKAGE-DAC-001 readiness refresh](s360-312-r4-fandac.md#package-dac-001-readiness-refresh)); the four still-blocking rows (3 / 6 / 8 / 10) in [`s360-312-r4-fandac.md` PACKAGE-DAC-001 readiness refresh](s360-312-r4-fandac.md#package-dac-001-readiness-refresh) must close before `PACKAGE-DAC-001-IMPLEMENT-001` can land. | `HW-PINMAP-312-FOLLOWUP` standalone reference doc *(landed 2026-05-22)* → `PACKAGE-DAC-001-READINESS-REFRESH` *(landed 2026-05-22 — this PR; docs / readiness only)* → DAC address / range / silkscreen evidence PR (provisionally `HW-PINMAP-312-FOLLOWUP-DAC-EVIDENCE-001` — GP8403 datasheet excerpt or DIP-switch ↔ I²C-address bench capture, `J2` / `J3` silkscreen reading + harness trace, operator/product output-range policy decision; `J7` pin-1 voltage-rail bench reading is a parallel `S360-100-BENCH-001` evidence track) → `S360-312` `schematic_status` promotion (separate JSON PR) → `PACKAGE-DAC-001-IMPLEMENT-001` (alias: `PACKAGE-GAP-001` FanDAC slice). |
...
- **What is wrong.** [`fan_gp8403.yaml`](../../packages/expansions/fan_gp8403.yaml)
  binds one GP8403 device only (`gp8403.id: fan_dac`,
  `address: ${fan_dac_address}` default `0x58` / alternate `0x59`)
  while the schematic + BOM show **two** `GP8403-TC50-EW` chips
  (`IC1` / `IC2`) on a shared I²C bus, each with independent
  DIP-switch address selection (`SW1` / `SW2` — BOM-confirmed
  `219-3MSTR` 3-pole SPST DIP switches from CTS). The DIP-switch
  position ↔ 7-bit I²C address mapping for each DAC is not
  recorded in the package and not measured on the bench. Core
  `J7` pin-1 is captured as `+5V` and module `J1` pin-1 is
  captured as `+3.3V`; the discrepancy is unresolved. UART-vs-
  Nextion arbitration on Module `J1` pins 4 / 5 (shared with the
  ESP32 boot-log path on `TXD0` / `RXD0`) is unresolved. The
  GP8403 output range (`${fan_dac_voltage_mode}` default `10V` /
  alternate `5V`) has **no** hardware jumper / solder-bridge on
  this board — the schematic ties each chip's `V5V` pin directly
  to `+12V` from the MT3608 boost, and the BOM contains no
  range-select hardware (per
  [`s360-312-r4-fandac.md` GP8403 output range capability](s360-312-r4-fandac.md#gp8403-output-range-capability)
  and [§BOM cross-check](s360-312-r4-fandac.md#bom-cross-check)),
  so range selection is firmware/register-driven only. The stale
  comment at [`fan_gp8403.yaml`](../../packages/expansions/fan_gp8403.yaml)
  file line 6 ("0-10V or 0-5V (jumper selectable on hardware)")
  is no longer corroborated by visible hardware. The active YAML
  body inherits `${fan_dac_i2c_id}: core_i2c` from the Core
  abstract package after `CORE-ABSTRACT-BUS-001B-IMPLEMENT-001`
  (PR #569).
- **Readiness refresh (2026-05-22).** The
  ...
  - **Still blocking**: row 3 (DIP-switch ↔ 7-bit I²C address
    truth table for `SW1` / `SW2`); row 6 (output-range policy
    decision — operator-requested firmware/product-selectable
    0–5 V / 0–10 V; package must not silently assume 10 V only);
    row 8 (`J2` / `J3` Cloudlift S12 silkscreen pin-1 identity +
    harness conductor trace); row 10 (package YAML correctness —
    implementation-pending; gated on rows 3, 6, and 8).
  - **Verdict**: `PACKAGE-DAC-001` is **not implementation-
    plannable yet**. The recommended next PR is a DAC address /
    range / silkscreen evidence PR — **not**
    `PACKAGE-DAC-001-IMPLEMENT-001`.
- **Allowed action now.** None on
  [`packages/expansions/fan_gp8403.yaml`](../../packages/expansions/fan_gp8403.yaml)
  or
  [`packages/expansions/fan_dac.yaml`](../../packages/expansions/fan_dac.yaml).
  `S360-312` JSON `schematic_status` is still
  `cataloged_unverified`; the four still-blocking rows (3 / 6 / 8 /
  10) in
  [`s360-312-r4-fandac.md` §PACKAGE-DAC-001 readiness refresh](s360-312-r4-fandac.md#package-dac-001-readiness-refresh)
  must close before any `fan_gp8403.yaml` body change. The
  standalone schematic+BOM reference doc landed on 2026-05-22
  and the readiness refresh records the recoverable evidence, but
  neither by itself unblocks the package.
- **Follow-up owner.** `HW-PINMAP-312-FOLLOWUP` standalone
  ...
  → DAC address / range / silkscreen evidence PR (provisionally
  `HW-PINMAP-312-FOLLOWUP-DAC-EVIDENCE-001` — GP8403 datasheet
  excerpt **or** DIP-switch ↔ I²C-address bench capture,
  `J2` / `J3` silkscreen reading + harness conductor trace, and
  operator / product output-range policy decision;
  `J7` pin-1 voltage-rail bench reading is a parallel
  `S360-100-BENCH-001` evidence track) + `S360-312`
  `schematic_status` promotion (separate JSON PR) →
  `PACKAGE-DAC-001-IMPLEMENT-001` / `PACKAGE-GAP-001` FanDAC
  slice. Subject to the `FanDAC` ↔ `AirIQ` mutex in
  [`config/webflash-compatibility.json`](../../config/webflash-compatibility.json).
...
| `PACKAGE-DAC-001-IMPLEMENT-001` (was `PACKAGE-DAC-001`) | done (HW-ASSETS-003) | `HW-PINMAP-312-FOLLOWUP` standalone reference doc *(landed 2026-05-22 at [`s360-312-r4-fandac.md`](s360-312-r4-fandac.md))* + `PACKAGE-DAC-001-READINESS-REFRESH` *(landed 2026-05-22 — this PR; [`s360-312-r4-fandac.md` §PACKAGE-DAC-001 readiness refresh](s360-312-r4-fandac.md#package-dac-001-readiness-refresh))* + DAC address / range / silkscreen evidence PR (provisionally `HW-PINMAP-312-FOLLOWUP-DAC-EVIDENCE-001`) closing rows 3 / 6 / 8 of the readiness refresh | `S360-312` `schematic_status` promotion (separate JSON PR) | required (Core `J7` vs Module `J1` rail silkscreen [`S360-100-BENCH-001` parallel track], `SW1` / `SW2` DIP-switch ↔ I²C address evidence per readiness refresh row 3, GP8403 datasheet excerpt **or** bench logic-analyser capture per row 3, Cloudlift S12 `J2` / `J3` silkscreen pin-1 + harness conductor trace per row 8, operator / product output-range policy decision per row 6, package-design decision among one-logical-FanDAC vs two-DAC-instances vs four-outputs per row 4 — note that row 9 UART0-vs-Nextion arbitration is deferred out of `PACKAGE-DAC-001` scope into a future product slice) | not applicable | resolved at the shared-I²C-bus-naming layer by `CORE-ABSTRACT-BUS-001B-IMPLEMENT-001` (PR #569); active YAML now inherits `${fan_dac_i2c_id}: core_i2c` |
