...
|---|---|---|---|---|---|---|---|
| [`packages/expansions/fan_relay.yaml`](../../packages/expansions/fan_relay.yaml) | `S360-310` Sense360 Relay | `FanRelay` — single on/off relay-driven fan via `${fan_relay_pin}` defaulting to `${relay_pin}` | [`s360-310-r4-relay.md`](s360-310-r4-relay.md) (HW-PINMAP-310) **`partial — schematic evidence available; package reconciliation pending`** (audit status promoted by HW-PINMAP-310-FOLLOWUP); module-side schematic PDF + curated artifact index committed under HW-ASSETS-310 at [`docs/hardware/artifacts/S360-310-R4.md`](artifacts/S360-310-R4.md) + [`schematics/S360-310-R4.pdf`](schematics/S360-310-R4.pdf) | `schematic-evidence-pending` + `needs-package-reconciliation` | `relay_pin` abstract-bus disagreement: Core schematic `Relay = IO3`; ceiling Core abstract `relay_pin: GPIO4` (`IO4 = SEN0609_RX`); generic Core abstract `relay_pin: GPIO10` (per [`s360-310-r4-relay.md` Reconciliation findings](s360-310-r4-relay.md#reconciliation-findings)). Module schematic confirms `J2` 3-pin "From Core" net order (`+5V` / `Relay` / `GND`) matches Core `J4`, and that the `Relay` net drives a `Q1` MMBT3904 NPN low-side switch (`R1` 1 kΩ base / `R2` 10 kΩ pull-down / `D1` flyback) into a `K1` mechanical relay — but **does not** establish which Core-side GPIO carries that net. Module-side silkscreen pin-1, `J1` `NO` / `COM` / `NC` mapping, `K1` relay part / contact rating, and Core-to-module harness identity all unknown. | **No package edit.** Schematic has arrived (HW-ASSETS-310) and HW-PINMAP-310-FOLLOWUP has consumed it; `CORE-ABSTRACT-BUS-001` + silkscreen / harness / BOM evidence still required before any package edit. | `HW-ASSETS-310` *(landed)* → `HW-PINMAP-310-FOLLOWUP` *(landed)* → silkscreen / harness / BOM evidence + `CORE-ABSTRACT-BUS-001` → `PACKAGE-RELAY-001` (alias: `PACKAGE-GAP-001` FanRelay slice). |
