...
- **Status.** `schematic-evidence-pending` + `needs-package-reconciliation` + `timing/compliance-pending` (compliance-gated). Status unchanged by HW-ASSETS-400.
- **What is wrong.** [`power_240v.yaml`](../../packages/hardware/power_240v.yaml)
  ...
  inrush, and EMI/EMC measurements are all owed to
  `HW-PINMAP-400-FOLLOWUP`. COMPLIANCE-001 mains-voltage UK / EU
  sign-off is a separate, additional gate before any `PWR`-bearing
  product / WebFlash work; it cannot be cleared by schematic evidence
  alone.
- **Allowed action now.** None on
...
- **Follow-up owner.** `HW-ASSETS-400` *(landed at
  [`docs/hardware/artifacts/S360-400-R4.md`](artifacts/S360-400-R4.md)
  and
  [`docs/hardware/schematics/S360-400-R4.pdf`](schematics/S360-400-R4.pdf))*
  → `HW-PINMAP-400-FOLLOWUP` (standalone reference doc + BOM-backed
  part-identity + silkscreen + creepage / clearance + thermal / EMI /
  EMC) → `S360-400` `schematic_status` promotion (separate JSON PR)
