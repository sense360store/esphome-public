...
- **Status.** `needs-package-reconciliation` + `evidence-complete` +
  `implementation-plannable` (rows 3 / 6 / 8 closed by
  `HW-PINMAP-312-FOLLOWUP-DAC-EVIDENCE-001`, 2026-05-22; the YAML
  reconciliation itself is `PACKAGE-DAC-001-IMPLEMENT-001`).
...
- **Readiness refresh (2026-05-22).** The
  ...
  - **Still implementation-pending**: row 10 (package YAML
    correctness — bind two GP8403s, per-chip address + range
    substitutions, four outputs, corrected jumper comment; this is
    `PACKAGE-DAC-001-IMPLEMENT-001`).
  - **Verdict**: `PACKAGE-DAC-001` is **now implementation-
    plannable**. The recommended next PR is
    **`PACKAGE-DAC-001-IMPLEMENT-001`**.
- **Allowed action now.** None on
  [`packages/expansions/fan_gp8403.yaml`](../../packages/expansions/fan_gp8403.yaml)
  or
  [`packages/expansions/fan_dac.yaml`](../../packages/expansions/fan_dac.yaml)
  in this evidence PR (both byte-identical). `S360-312` JSON
  `schematic_status` is still `cataloged_unverified`. With rows
  3 / 6 / 8 closed, the YAML reconciliation may now proceed as
  `PACKAGE-DAC-001-IMPLEMENT-001`. Residual product / bench items
  ...
  tracked separately and do **not** block the package YAML.
- **Follow-up owner.** `HW-PINMAP-312-FOLLOWUP` standalone
  ...
  slice (bind two GP8403s, per-chip address + range substitutions,
  four outputs, corrected jumper comment) → `S360-312`
  `schematic_status` promotion (separate JSON PR). Residual product
