...
| Sense360 Relay | `S360-310` | Core-side `J4` connector documented in [`s360-100-r4-core.md` J4](s360-100-r4-core.md#j4--relay-module-connector-3-pin): 3-pin (`+5V`, `Relay`, `GND`), drive signal `Relay` from ESP32 `IO3`. **Module-side schematic now committed under HW-ASSETS-310** at [`docs/hardware/schematics/S360-310-R4.pdf`](schematics/S360-310-R4.pdf), with curated artifact index at [`docs/hardware/artifacts/S360-310-R4.md`](artifacts/S360-310-R4.md) (records `J2` 3-pin "From Core" `+5V` / `Relay` / `GND` matching Core `J4`; `K1` mechanical relay with `Q1` MMBT3904 NPN low-side coil driver + `R1` 1 kΩ base / `R2` 10 kΩ pull-down / `D1` flyback diode; `J1` 3-pin "Inline Fan" load-side; no opto, no indicator LED, no snubber, no relay part number visible). HW-PINMAP-310 audit doc at [`s360-310-r4-relay.md`](s360-310-r4-relay.md) **status unchanged**: `pending — schematic/design evidence required`. Firmware package: [`packages/expansions/fan_relay.yaml`](../../packages/expansions/fan_relay.yaml). Not in Release-One. Catalog `schematic_status` for `S360-310` is unchanged (`cataloged_unverified`; no `schematic_file` set). | `partially-documented`, `not-needed-for-release-one` | Not required (no `FanRelay` in Release-One). | Standalone schematic-backed reference doc, silkscreen / bench reconciliation, package YAML reconciliation, and abstract-bus rebind owed to `HW-PINMAP-310-FOLLOWUP` and `CORE-ABSTRACT-BUS-001` before any `FanRelay`-bearing config string ships as stable. |
...
- **Evidence available.** Core `J4` connector (3-pin, `+5V` / `Relay` /
  ...
  is the on/off relay driver package. **Module-side schematic now
...
- **Evidence missing.** Standalone schematic-backed reference doc;
  silkscreen pin-1 verification on `J1` / `J2`; `J1` `NO` / `COM` /
  `NC` mapping; `K1` relay part identity, coil voltage, and contact
  rating (BOM-dependent); Core-to-module harness identity.
- **Audit doc.** The HW-PINMAP-310 per-board audit record at
  ...
  silkscreen / bench reconciliation, and the package / abstract-bus
  reconciliation are owed to `HW-PINMAP-310-FOLLOWUP` and
  ...
  `relay_pin` disagreement remains **unresolved** by HW-ASSETS-310 —
  the module schematic confirms only that the module receives the
  `Relay` net on `J2` pin 2 and drives a 5 V coil via `Q1`; it does
  not establish which Core-side GPIO physically connects to that net.
  Resolution belongs to `HW-PINMAP-310-FOLLOWUP` and the systemic
  Core abstract-bus rebind.
