...
| K2 | esphome-public | Promote Kitchen bundle `S360-KIT-KITCHEN-P` stable-candidate → stable-release in `config/room-bundle-skus.json`, clearing the covered G8 evidence gate(s); align catalog/build status fields for `Ceiling-POE-AirIQ-RoomIQ`; contract tests updated. Gated on the completed, owner-attested proof. Held PR. | PENDING — awaiting bench proof |
