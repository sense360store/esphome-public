...
| PWM-12 | TachIO / GPIO16 + RPM | NEEDS BENCH / deferred | No `pulse_counter`; per-fan RPM via SX1509 `pulse_counter` is compile-proven unsupported (`[sx1509] is an invalid option for [pin]`); architectural rule pinned by [S360-100-NATIVE-TACH-PULSE-001](hardware/s360-100-native-tach-pulse-strategy.md) — **tach / pulse-counter inputs must terminate on native ESP32 GPIO; the SX1509 expander must not be used for tach** (PWM-drive output via SX1509 is a separate capability and stays supported). **R4 refresh (2026-05-28):** the new canonical `S360-100-R4.pdf` now prints `TachIO` / `Pul_Cou1..4` / `TachPMW1..4` directly against native ESP32-S3 module pins — schematic-side termination is now visibly rule-compliant ([`s360-100-core-architecture.md` § Pin allocation table](hardware/s360-100-core-architecture.md#pin-allocation-table--native-esp32-s3-gpio-termination)); firmware-binding and bench-measured RPM remain **out of scope** for this row. **S360-100-NATIVE-FAN-GPIO-MAP-001 (this PR, 2026-05-28):** the canonical FanPWM control + tach GPIO map is recorded in [`s360-100-native-fan-gpio-map.md`](hardware/s360-100-native-fan-gpio-map.md) (`TachPMW1..4` -> `IO10` / `IO11` / `IO12` / `IO39`; `Pul_Cou1..4` -> `IO17` / `IO18` / `IO46` / `IO9`; `TachIO` -> `IO16`); the SX1509-routed FanPWM control / tach path is classified legacy / superseded and `packages/expansions/fan_pwm.yaml` + `packages/expansions/fan_pwm_sx1509.yaml` carry that classification in their headers. The firmware re-bind to the native map, the `esphome compile` of the re-bind, and the bench session that captures measured PWM / current / thermal / RPM remain out of scope for this row. | PWM-SX1509-TACH-PROOF-001 / PR #589; PACKAGE-PWM-TACH-STRATEGY-001 / PR #588; S360-100-NATIVE-TACH-PULSE-001 / PR #634; S360-100-NATIVE-TACH-PULSE-001 — R4 refresh (PR #635); S360-100-NATIVE-FAN-GPIO-MAP-001 (this PR) | No (kept false) | keep `rpm_supported: false`; any RPM still needs a separate firmware-binding PR + a bench session that captures measured RPM on the schematic-printed native-GPIO termination | COMPONENT-NATIVE-TACH-001 (future) |
