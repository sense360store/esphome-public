...
| PWM-16 | Native ESP32-S3 GPIO FanPWM YAML candidate (re-bind off SX1509) | NATIVE CANDIDATE ADDED (compile pending CI) | Native package [`packages/expansions/fan_pwm_native.yaml`](../packages/expansions/fan_pwm_native.yaml) binds `TachPMW1..4`→`IO10`/`IO11`/`IO12`/`IO39` via `output: platform: ledc` (NO SX1509) and `Pul_Cou1`/`Pul_Cou2`/`Pul_Cou4`→`IO17`/`IO18`/`IO9` via internal-diagnostic `sensor: platform: pulse_counter` (NO SX1509); `Pul_Cou3`/`IO46` disabled/TBD (collides with Core `fan_status_led_pin`/`GPIO46`), `TachIO`/`IO16` reserved/pending. Compile-only skeleton [`products/compile-only/ceiling-poe-fanpwm-native.yaml`](../products/compile-only/ceiling-poe-fanpwm-native.yaml) + target `ceiling-poe-fanpwm-native-compile-only` (`compile_validation_status: pending-ci`, `rpm_supported: false`). No native `esphome compile` run performed (legacy SX1509 run `26414398902` does not transfer); no compile-proven firmware claimed. No WebFlash wrapper / build-matrix / artifact. Pinned by [`test_native_fanpwm_yaml.py`](../tests/test_native_fanpwm_yaml.py). | S360-311-NATIVE-FANPWM-YAML-001 (this PR) | No (candidate only) | run full `esphome compile` in CI against the native skeleton (record honestly, do not claim until green); RPM/tach + current/thermal stay carried by `PWM-6` / `PWM-12` / `PWM-13` | full compile in CI; then `S360-311-CURRENT-THERMAL-001` |
