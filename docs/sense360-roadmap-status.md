...
| `Ceiling-POE-AirIQ-RoomIQ` | **preview** | 1.0.10 | `Sense360-Ceiling-POE-AirIQ-RoomIQ-v1.0.10-preview.bin` | Kitchen firmware (`S360-KIT-KITCHEN-P`). Promoted to stable v1.0.6 (2026-06-09) under owner risk-acceptance waiver `HW-AIRIQ-WAIVER-2026-06`, then **deliberately demoted to preview by PR #834** (2026-07-16: the owner waiver is not hardware verification and must not cut a stable build). The owner decision of 2026-07-28 (SENSE360-CANONICALISATION-001) **upholds #834** and withdraws the same-day promotion decision as founded on a false premise: the catalogue stays preview, and promotion to production is gated on the bench attestation #834 requires (owner-authored, never machine-written). The published GitHub Release `v1.0.9` and its `-stable.bin` asset (security rebuild, 2026-07-06) remain immutable and available; the WebFlash *presentation* moves to preview so served presentation matches this recorded channel. Declared next artifact moved v1.0.9 -> **v1.0.10** by `SENSE360-REVIEW-RELEASE-001`: the v1.0.9 release predates PR #875, so its published binary cannot carry the accepted review entity contract, and the published v1.0.9 assets are an immutable fact — the next artifact under the preview channel's canonical naming therefore takes a new version. This is a **declaration** bump only: `release_state` stays `metadata-ready-unpublished`, and no binary, GitHub Release, tag, or manifest is cut by it. Bundle stays hidden / not buyable. |
...
| `Ceiling-POE-AirIQ-FanRelay-RoomIQ` | **experimental** | 1.0.1 | `Sense360-Ceiling-POE-AirIQ-FanRelay-RoomIQ-v1.0.1-experimental.bin` | Kitchen FanRelay room bundle (S360-310, **mains-adjacent contact switching**). Owner-declared **experimental-only** row (`HW-RELEASE-001`; lane posture per `COMPLIANCE-001`); metadata-ready / unpublished. Competent-person installation where mains switching is in scope. **Never stable / recommended / default / buyable / kit-exposed.** Declared next artifact moved v1.0.0 -> **v1.0.1** by the owner decision of 2026-07-28 (SENSE360-CANONICALISATION-001): the published `v1.0.0-preview` artifact for this config is an immutable fact, so the next artifact under the experimental channel's canonical naming takes a new version — no second name for v1.0.0 ever. |
| `Ceiling-POE-VentIQ-FanRelay-RoomIQ` | **experimental** | 1.0.1 | `Sense360-Ceiling-POE-VentIQ-FanRelay-RoomIQ-v1.0.1-experimental.bin` | Bathroom FanRelay room bundle (S360-310, **mains-adjacent contact switching**). Owner-declared **experimental-only** row (`HW-RELEASE-001`; lane posture per `COMPLIANCE-001`); metadata-ready / unpublished. Competent-person installation where mains switching is in scope. **Never stable / recommended / default / buyable / kit-exposed.** Declared next artifact moved v1.0.0 -> **v1.0.1** by the owner decision of 2026-07-28 (SENSE360-CANONICALISATION-001): the published `v1.0.0-preview` artifact for this config is an immutable fact, so the next artifact under the experimental channel's canonical naming takes a new version — no second name for v1.0.0 ever. |
...
| Next publish action | **owner-reserved**: `Ceiling-POE-AirIQ-RoomIQ` is declared at **1.0.10** on the preview channel awaiting the owner "Release: Create GitHub Release" dispatch (tag `v1.0.10-preview`, prerelease). Scope note — `SENSE360-REVIEW-RELEASE-001` bumped **only** the review composition. PR #875 changed shared packages (`packages/boards/s360-100-core-ceiling.yaml`, `packages/boards/s360-410-poe-psu.yaml`, `packages/features/presence_framework.yaml`), and the generated entity tables moved for **all four** served configs, so the served binaries for `Ceiling-POE-VentIQ-RoomIQ` (stable 1.0.7), `Ceiling-POE-RoomIQ` (stable 1.0.8) and `Ceiling-POE-VentIQ-RoomIQ-LED` (preview 1.0.1) also predate #875. Whether to re-cut those is an **open owner decision**, not a planned action — a stable re-cut in particular is a Release-One change and is owner-only. Recorded here so the gap is visible, not resolved. |
