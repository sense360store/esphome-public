...
| **Ceiling-POE-RoomIQ** | `products/sense360-ceiling-poe-roomiq.yaml` | S360-100 + S360-200 + S360-410 | `blocked` (`PRODUCT-POE-410-001`) | compile-only-ci (`pending-ci`; top-level target `ceiling-poe-roomiq-product-compile-only`) | none | none | **none** (target `stable-candidate`) | Authored by **V1-R4-CREATE-001** (the `S360-KIT-BEDROOM-P` firmware); RoomIQ-only PoE ceiling stack, the validated base for the AirIQ / LED stacks. Release-blocked on the shared `S360-410` schematic-verification chain (`PRODUCT-POE-410-001` / `PACKAGE-POE-410-001`): Core + RoomIQ boards verified, `S360-410` PoE PSU `cataloged_unverified`. Stable promotion owned by `STABLE-TARGET-ROOMIQ-001`. Not WebFlash-exposed; not production / preview / verified / release-ready; no `artifact_name`, no wrapper, no `config/webflash-builds.json` row. |
