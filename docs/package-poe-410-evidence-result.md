...
| **PCB source / gerbers** | ❌ none committed | Drive `S360-410-R4 / gerbers` (retained-but-not-committed) | No `*.kicad_pcb` or gerbers archive committed; storage-backend decision still owed per [Hardware Artifact Policy §Future storage decision](hardware/hardware-artifact-policy.md#future-storage-decision). |
| **Prior PR evidence** | ✅ recorded | HW-ASSETS-410 / PR #516; HW-PINMAP-410-FOLLOWUP / PR #517; HW-BOM-ASSETS-002 / PR #535; PACKAGE-POE-410-001 docs-only PR #526; package-header cleanup PR #538; PRODUCT/WEBFLASH/RELEASE-POE-410-001 docs-only investigations PR #528 / #530 / #532 | The schematic, BOM, topology, and package-header-cleanup work is landed; the `verified` JSON promotion and all bench/PCB/silkscreen/harness evidence remain owed. |
...
| **PCB completeness** | None committed. | KiCad PCB source (`S360-410-R4.kicad_pcb`) and/or gerbers; needed to verify isolation-barrier creepage/clearance and `H1`–`H4` bonding. | ❌ missing (E13) | Blocks isolation-geometry verification → contributes to the `verified` gate; does not by itself block a no-new-entry caveat closure. |
...
| **Manufacturing readiness** | Drive CAD/manufacturing sets present (sch_pdf / bom / cpl / gerbers / step / images). | Committed PCB/gerbers; `R4` silkscreen P/N / REV / date-code (tracker `G01` Waiting on logo SVG `N01`); PoE PSU rename (tracker `R11` To do). | ❌ missing (E9/E13) | Blocks manufacturing-evidence closure; contributes to `verified` and Release-One caveat closure. |
...
**Open / missing:** PCB source (E13), connector silkscreen (E9), J2
...
manufacturing silkscreen (E9/E13).
...
| **E13 — PCB source / gerbers** | `S360-410-R4.kicad_pcb` and/or gerbers archive, ingested once a storage backend is chosen per [Hardware Artifact Policy §Future storage decision](hardware/hardware-artifact-policy.md#future-storage-decision). | Isolation-barrier creepage/clearance and `H1`–`H4` PCB bonding verifiable from source. |
...
7. Hi-pot the `F0505S-2WR2` isolation barrier; measure insulation
   ...
   continuity. *(E12)*
8. Supply `S360-410-R4.kicad_pcb` and/or gerbers once a storage
   backend is chosen. *(E13)*
...
> (E9), J2-harness (E10), and PCB-source (E13) evidence classes are
> **not on file**. This PR records the consolidated evidence matrix,
