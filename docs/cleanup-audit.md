...
| `docs/webflash-release-proof.md` | `current` | ESP-006 / ESP-007 proof record (run `25763009641`). RELEASE-003 extends this doc with a new "LED preview release proof (RELEASE-003)" section that records the LED preview proof scaffolding: a pending-proof table whose static fields are known from `config/webflash-builds.json` (config string `Ceiling-POE-VentIQ-RoomIQ-LED`, channel `preview`, version `1.0.0`, artifact `Sense360-Ceiling-POE-VentIQ-RoomIQ-LED-v1.0.0-preview.bin`, matrix product `ceiling-poe-ventiq-roomiq-led`, build file `products/sense360-ceiling-poe-ventiq-roomiq-led.yaml`) and whose dynamic fields (tag, run URL, asset size, SHA256, validator/asset-check pass states) are all marked `pending`; an operator runbook describing the future steps (generate the release-notes draft, replace the Changelog TODO, tag a GitHub `prerelease`, let `.github/workflows/firmware-build-release.yml` build and attach the LED preview artifact, back-fill the table); a `- [ ]` proof checklist (GitHub Release tag, Actions run URL, artifact filename, artifact size, SHA256, release-body validation command, generated release-notes command, WebFlash import source URL, explicit Release-One-unchanged statement); and a "What RELEASE-003 does NOT do" guardrail block. RELEASE-003 is documentation-only — it does **not** build firmware, tag a release, commit `.bin` files, claim proof exists, promote `Ceiling-POE-VentIQ-RoomIQ-LED` to `production`, alter Release-One (`Ceiling-POE-VentIQ-RoomIQ`, `Sense360-Ceiling-POE-VentIQ-RoomIQ-v1.0.0-stable.bin`, tag `v1.0.0`, run `25763009641`), unblock FanTRIAC, or import anything into WebFlash. Cross-linked from `docs/product-led-preview-decision.md` (PRODUCT-009 outstanding-proof paragraph) and `docs/product-onboarding.md` (LED guardrail bullet). | Keep. |
