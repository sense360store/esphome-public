...
(itself now additionally gated on a **`FW-COMPILE-DAC-001`** compile-only
validation — the `gp8403:` `voltage:` field is fed the neutral `0-10V`
value rather than ESPHome's bare `10V` / `5V` enum, and with no FanDAC
compile-only target this is **unvalidated**), which is upstream of
...
**landed PR #573**) → `FW-COMPILE-DAC-001` (compile-only validation of
the `voltage:` binding; confirm `0-10V` validates or record the
`0-10V` → `10V` fix) → `PRODUCT-DAC-001` (canonical product YAML;
outcome-first user-facing names; carry the `J2` / `J3` harness-trace +
`J3` silkscreen-transposition caveats; Nextion / `J7` out of scope
unless the product drives a display; enforce mutex; no AirIQ-bearing
variant) → `WEBFLASH-DAC-001` (wrapper + catalog + build-matrix on
