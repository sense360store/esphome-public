...
`blocker: HW-005`, `webflash_build_matrix: false`. No FanTRIAC
artifact has ever been built, signed, attached, or imported. The
TRIAC board has its module-side schematic PDF committed at
...
The reach to `advanced/manual-warning-artifact-only` requires, in
order: `HW-005` unblock (S360-320 schematic-side electrical /
firmware-side timing reconciliation; per
...
policy to `advanced/manual-warning-only`; preserve all
...
`WF-TRIAC-001` (advanced / manual-warning WebFlash wrapper /
catalog / build-matrix entry, behind the manual-warning UX gate) →
`RELEASE-TRIAC-001` (build the advanced-channel `.bin`; sign;
...
they are `docs-only` / `legacy-only`. The PWR-240V board has no
committed module-side schematic PDF;
[`hardware/s360-400-r4-power.md`](hardware/s360-400-r4-power.md)
status is `pending — schematic / design evidence required`.
COMPLIANCE-001 (mains-voltage UK / EU) is in force.
...
→ `WEBFLASH-POWER-400-001` → `RELEASE-POWER-400-001` (build / sign
/ attach the `.bin`; release notes; checksums; proof row) →
`WF-IMPORT-GAP-001`.
...
artifact. The Release-One product `Ceiling-POE-VentIQ-RoomIQ` and
the LED preview `Ceiling-POE-VentIQ-RoomIQ-LED` both consume
S360-410 logically under the existing schematic-pending caveat
preserved in
...
`HW-ASSETS-410` → `HW-PINMAP-410-FOLLOWUP` →
`S360-100-BENCH-001` update / `HW-002 OQ#6` closure →
