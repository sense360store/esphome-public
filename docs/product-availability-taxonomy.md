...
- **`blocked`** is a hold at the rung the entry currently occupies.
  ...
  **`blocked` does not mean removed or hidden.** A `blocked`
  hardware-evidence or compliance status holds an entry below its
  next ladder rung; it does **not** authorise removing the SKU, the
  package, the product YAML, or the WebFlash-wrapper reference from
  the repo. Repository inclusion is a documentation /
  engineering-record decision and is independent of compliance
  certification. FanTRIAC is the canonical example: its catalog row
  stays `status: blocked` under HW-005 with `webflash_build_matrix:
  false`, while its intended long-term posture is **advanced /
  manual-warning candidate** — policy-recorded by `PRODUCT-TRIAC-001`
  as a notes-only catalog edit (see
  ...
  The two qualifiers are distinct: `blocked-from-standard-exposure`
  is a **current state** (the SKU must not appear on Release-One,
  REQUIRED_CONFIGS, kit / default, recommended, or
  compliance-certified surfaces today), while
  `advanced/manual-warning-only` is the **long-term posture** for
  the eventual WebFlash exposure class behind a manual-warning UX
  gate — never standard, never recommended, never kit / default,
  never compliance-certified by virtue of having a build artifact.
  Both qualifiers can be carried simultaneously, as they are for
  FanTRIAC today.
...
| `S360-311` Sense360 PWM | `cataloged-unverified` JSON / `partially-documented` audit (Core-side `J6` 13-pin connector captured; pin-order **verify**) | `pin-map-pending` for an artifact index | `partially-documented`, `not-needed-for-release-one` | [`packages/expansions/fan_pwm.yaml`](../packages/expansions/fan_pwm.yaml) exists; `package-yaml-pending` / `needs-package-reconciliation` per [`hardware/s360-311-r4-pwm.md` Package YAML status](hardware/s360-311-r4-pwm.md#package-yaml-status); not reconciled against module-side schematic | No product YAML consumes this board today (`firmware-missing` for any FanPWM-bearing config) | `firmware-build-pending` / `design-pending` for any FanPWM-bearing config | Productization pending: HW-PINMAP-311 audit doc landed at [`hardware/s360-311-r4-pwm.md`](hardware/s360-311-r4-pwm.md) with **status: `partial — schematic evidence available; package reconciliation pending`**; records the SX1509-channel vs direct-ESP32-GPIO mapping disagreement, the UART-on-`J3`-pins-11/12 routing question, and the `"NINE 4pin FANs"` documentation question as unresolved. J6 pin-order **verify** must resolve against silkscreen. Follow-ups: `HW-PINMAP-311-FOLLOWUP`, `PACKAGE-GAP-001` FanPWM slice — per [`hardware/s360-311-r4-pwm.md` Follow-up PRs](hardware/s360-311-r4-pwm.md#follow-up-pr-sequence) and [HW-ASSETS-001 Follow-up PR sequence](hardware/hardware-artifact-policy.md#follow-up-pr-sequence). |
| `S360-312` Sense360 DAC | `cataloged-unverified` JSON / `partially-documented` audit (Core-side `J7` 6-pin connector fully captured) | `pin-map-pending` for an artifact index | `partially-documented`, `not-needed-for-release-one` | [`packages/expansions/fan_gp8403.yaml`](../packages/expansions/fan_gp8403.yaml) exists; `package-yaml-pending` / `needs-package-reconciliation` per [`hardware/s360-312-r4-dac.md` Package YAML status](hardware/s360-312-r4-dac.md#package-yaml-status); not reconciled against module-side schematic | No product YAML consumes this board today (`firmware-missing` for any FanDAC-bearing config) | `firmware-build-pending` / `design-pending` for any FanDAC-bearing config | Productization pending: HW-PINMAP-312 audit doc landed at [`hardware/s360-312-r4-dac.md`](hardware/s360-312-r4-dac.md) with **status: `partial — schematic evidence available; package reconciliation pending`**; records the Core `J7` pin-1 `+5V` vs Module `J1` pin-1 `+3.3V` voltage-rail discrepancy, the DIP-switch I²C address-selection scheme on the two `GP8403-TC50-EW` DACs (`IC1` / `IC2`), the UART0-vs-Nextion arbitration question on Module `J1` pins 4 / 5, and the stale header-comment connector / GPIO claims in [`packages/expansions/fan_gp8403.yaml`](../packages/expansions/fan_gp8403.yaml) as unresolved. FanDAC ↔ AirIQ conflict enforced in [`config/webflash-compatibility.json`](../config/webflash-compatibility.json). Follow-ups: `HW-PINMAP-312-FOLLOWUP`, `PACKAGE-GAP-001` FanDAC slice — per [`hardware/s360-312-r4-dac.md` Follow-up PRs](hardware/s360-312-r4-dac.md#follow-up-pr-sequence) and [HW-ASSETS-001 Follow-up PR sequence](hardware/hardware-artifact-policy.md#follow-up-pr-sequence). |
...
| `S360-320` Sense360 TRIAC | `cataloged-unverified` JSON / `blocked` audit (HW-005 + COMPLIANCE-001) | `pin-map-pending` for an artifact index (HW-ASSETS-003 module-side schematic PDF + artifact index committed at [`docs/hardware/schematics/S360-320-R4.pdf`](hardware/schematics/S360-320-R4.pdf) + [`docs/hardware/artifacts/S360-320-R4.md`](hardware/artifacts/S360-320-R4.md)) | `blocked` (HW-005); also gated by COMPLIANCE-001 | [`packages/expansions/fan_triac.yaml`](../packages/expansions/fan_triac.yaml) retained as **blocked / reference** with BLOCKED / UNVERIFIED banner; `package-yaml-pending` / `needs-package-reconciliation` per [`hardware/s360-320-r4-triac.md` Package YAML status](hardware/s360-320-r4-triac.md#package-yaml-status) | `Ceiling-POE-VentIQ-FanTRIAC-RoomIQ` exists as `status: blocked`, `blocker: HW-005`, `webflash_build_matrix: false`; YAML / wrapper retained as reference only | `firmware-build-pending` / `blocked` | **Stays blocked.** Productization pending: HW-PINMAP-320 audit doc landed at [`hardware/s360-320-r4-triac.md`](hardware/s360-320-r4-triac.md) with **status: `partial — schematic evidence available; package reconciliation, timing validation, and compliance/certification pending`**; records the `TRI_GPIO*` (Core) vs `ESP_GPIO*` (Module) naming divergence, the `ac_dimmer`-vs-SX1509 timing constraint, and the intended advanced / manual-warning long-term product posture (visible / selectable, buildable after package evidence, installable only through an advanced / manual-warning path; **not** Release-One, **not** REQUIRED_CONFIGS, **not** recommended, **not** kit / default, **not** compliance-certified). The advanced / manual-warning posture is **policy-recorded** by `PRODUCT-TRIAC-001` as a notes-only catalog edit on `Ceiling-POE-VentIQ-FanTRIAC-RoomIQ` in [`config/product-catalog.json`](../config/product-catalog.json); the JSON lifecycle row stays `blocked`, `blocker: HW-005`, `webflash_build_matrix: false`, no `artifact_name`, and no new lifecycle enum was added. HW-005 missing-evidence checklist must be satisfied **and** COMPLIANCE-001 mains-voltage review must clear before any product / WebFlash work; the SX1509-only hypothesis is rejected (the on-board controller IC Option (b) is eliminated for this revision per the module-side schematic). Follow-ups: `HW-PINMAP-320-FOLLOWUP`, `PACKAGE-TRIAC-001`, `PRODUCT-TRIAC-001`, `PRODUCT-TRIAC-002`, `WF-TRIAC-001`, `RELEASE-TRIAC-001`, `WF-IMPORT-TRIAC-001`, `COMPLIANCE-001`, `HW-005`, `HW-CATALOG-320` — per [`hardware/s360-320-r4-triac.md` Follow-up PR sequence](hardware/s360-320-r4-triac.md#follow-up-pr-sequence). |
...
| `S360-410` Sense360 PoE PSU | `cataloged-unverified` JSON / `partially-documented` audit (Core-side `J2` `PoE_ACDC` inlet captured; module-side schematic committed under HW-ASSETS-410 / PR #516; HW-PINMAP-410-FOLLOWUP audit `partial`) | `pin-map-pending` for an artifact index | `partially-documented` (`J2` harness identity remains HW-002 Open Question #6) | [`packages/hardware/power_poe.yaml`](../packages/hardware/power_poe.yaml) is a logical PoE-power package emitting diagnostic sensors only; `package-yaml-ready` (logical, no GPIO binding) | `production` via Release-One — the "schematic verification pending" caveat is preserved in the catalog `notes` and in [`release-one-hardware-audit.md`](release-one-hardware-audit.md) | `stable-available` (consumed by Release-One); `preview-available` (consumed by the LED preview) | **In Release-One under a `partially-documented` evidence state.** Module-side schematic committed under HW-ASSETS-410 / PR #516; HW-PINMAP-410-FOLLOWUP has consumed it and promoted the HW-PINMAP-410 audit doc to `partial — schematic evidence available; package reconciliation, PoE PD controller / magnetics / buck / isolated DC/DC / harness identity evidence pending`. Standalone schematic-backed reference-doc rewrite, BOM-backed package-header reconciliation (`PACKAGE-POE-410-001`), and HW-002 OQ#6 / `S360-100-BENCH-001` J2-harness closure are still owed. The caveat is preserved verbatim, not promoted away. |
