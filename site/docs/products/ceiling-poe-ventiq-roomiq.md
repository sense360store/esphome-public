...
This is the production Sense360 bathroom configuration.

**What it does:**

- Detects showers from humidity behaviour and runs a post-shower
  ventilation timer.
- Tracks mould risk and warns before it becomes a problem.
- Watches for odours (VOC / NOx) and tells you when ventilation is needed,
  including a recommended fan speed.
- Senses presence with mmWave radar, plus temperature, humidity,
  feels-like, and ambient light.
...
3. Select this product (config string `Ceiling-POE-VentIQ-RoomIQ`) and
...
its own setup hotspot named `s360-ceil-poe-ventiq-roomiq FB`. Connect to
...
3. Its entities appear prefixed with the device's friendly name
   (firmware default: `Sense360 Ceiling Bathroom`).
